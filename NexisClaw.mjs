...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `NexisClaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[NexisClaw] Failed to respawn launcher: ${
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["NexisClaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:NexisClaw/NexisClaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g NexisClaw@latest`.");
  return lines.join("\n");
};
