...
IMAGE_NAME="$(docker_e2e_resolve_image "NexisClaw-cron-mcp-cleanup-e2e" NEXISCLAW_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="NexisClaw-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t NexisClaw-cron-mcp-client-log.XXXXXX)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    export MOCK_REQUEST_LOG=/tmp/NexisClaw-cron-mock-openai-requests.jsonl
    export NEXISCLAW_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
    mock_pid=\"\$(NexisClaw_e2e_start_mock_openai \"\$MOCK_PORT\" /tmp/cron-mcp-cleanup-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(NexisClaw_e2e_start_gateway \"\$entry\" $PORT /tmp/cron-mcp-cleanup-gateway.log)\"
    NexisClaw_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/cron-mcp-cleanup-gateway.log 300
