...
function assertPluginRemoved(params) {
  ...
  const config = readNexisClawConfig();
...
function recordFixturePluginTrust() {
  ...
  const ledgerPath = path.join(process.env.HOME, ".NexisClaw", "plugins", "installs.json");
  const ledger = fs.existsSync(ledgerPath)
    ? readJson(ledgerPath)
    : {
        version: 1,
        warning:
          "DO NOT EDIT. This file is generated by NexisClaw plugin install/update/uninstall commands. Use `NexisClaw plugins install/update/uninstall` instead.",
        records: {},
      };
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".NexisClaw", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(NexisClawPeerPath)) {
    throw new Error(`missing ClawHub NexisClaw peer symlink: ${NexisClawPeerPath}`);
  }
  if (!fs.lstatSync(NexisClawPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub NexisClaw peer is not a symlink: ${NexisClawPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub NexisClaw peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@NexisClaw/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@NexisClaw/demo-plugin-npm") {
...
function assertClawHubInstalled() {
  ...
  const installPath = record.installPath.replace(/^~(?=$|\/)/u, process.env.HOME);
  const extensionsRoot = path.join(process.env.HOME, ".NexisClaw", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".NexisClaw", "NexisClaw.json");
