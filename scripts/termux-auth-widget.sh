...
# NexisClaw Auth Widget for Termux
...
# Check auth status
termux-toast "Checking NexisClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/NexisClaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/NexisClaw/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/NexisClaw/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: NexisClaw won't work until re-auth"
                ;;
        esac
        ;;
