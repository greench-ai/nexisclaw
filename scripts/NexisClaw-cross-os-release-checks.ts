...
const PUBLISHED_INSTALLER_BASE_URL = "https://NexisClaw.ai";
...
const CROSS_OS_AGENT_TURN_OPTIONAL = parseBooleanEnv("NEXISCLAW_CROSS_OS_AGENT_TURN_OPTIONAL", true);
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.NexisClaw-install-stage(?:-[^/]+)?$/iu;
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `NexisClaw@${previousVersion}` : "NexisClaw@latest");
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    await runTimedLanePhase(lane, "update", async () => {
      try {
        updateResult = await runNexisClaw({
          lane,
          env: updateEnv,
          args: updateArgs,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `NexisClaw-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".NexisClaw");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    NEXISCLAW_CONFIG_PATH: join(lane.stateDir, "NexisClaw.json"),
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]NexisClaw\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/NexisClaw[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "NexisClaw@latest") {
    return "";
  }
  if (trimmed.startsWith("NexisClaw@")) {
    return trimmed.slice("NexisClaw@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "NexisClaw@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view NexisClaw@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'NexisClaw.cmd'),
      (Join-Path $npmPrefix 'NexisClaw.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command NexisClaw -ErrorAction Stop
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    if (!cliPath) {
      throw new Error("Failed to resolve installed NexisClaw path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v NexisClaw >/dev/null 2>&1",
  ...
  if (!cliPath) {
    throw new Error("Failed to resolve installed NexisClaw path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `NexisClaw update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.NexisClaw/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "NexisClaw/plugin-sdk/plugin-runtime",
) {
  return `
...
async function runAgentTurn(params) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const sessionId = `cross-os-release-check-${params.label}-${Date.now()}-${attempt}`;
    try {
      const result = await runNexisClaw({
        lane: params.lane,
        env: params.env,
        args: buildReleaseAgentTurnArgs(sessionId),
        logPath: params.logPath,
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>NexisClaw Control</title>") &&
          html.includes("<NexisClaw-app></NexisClaw-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>NexisClaw Control</title>")} app=${html.includes("<NexisClaw-app></NexisClaw-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runNexisClaw(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "NexisClaw.mjs");
}
