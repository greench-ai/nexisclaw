...
function usage() {
  return "Usage: node scripts/check-NexisClaw-package-tarball.mjs <NexisClaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`NexisClaw package tarball does not exist: ${tarball}`);
}
...
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-NexisClaw-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "NexisClaw-package-tarball-"));
try {
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`NexisClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`NexisClaw package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("NexisClaw package tarball integrity passed.");
