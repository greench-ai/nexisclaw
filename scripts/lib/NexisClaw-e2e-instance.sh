...
NexisClaw_e2e_eval_test_state_from_b64() { eval "$(printf '%s' "${1:?missing NexisClaw test-state script}" | base64 -d)"; }
NexisClaw_e2e_resolve_entrypoint() {
  local entry
  for entry in dist/index.mjs dist/index.js; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "NexisClaw entrypoint not found under dist/" >&2
  return 1
}
NexisClaw_e2e_package_root() {
  local prefix="${1:-}"
  if [ -n "$prefix" ]; then
...
NexisClaw_e2e_package_entrypoint() {
  local root="${1:?missing package root}"
  local entry
  for entry in "$root/dist/index.mjs" "$root/dist/index.js"; do
    [ -f "$entry" ] && { printf '%s\n' "$entry"; return 0; }
  done
  echo "NexisClaw package entrypoint not found under $root/dist/" >&2
  return 1
}
NexisClaw_e2e_install_package() {
  local log_file="$1"
  local label="${2:-mounted NexisClaw package}"
  local prefix="${3:-}"
...
NexisClaw_e2e_assert_package_extensions() {
  local root="$1"
  shift
  local extension
  for extension in "$@"; do
...
NexisClaw_e2e_write_state_env() {
  local target="${1:-/tmp/NexisClaw-test-state-env}"
  {
...
NexisClaw_e2e_install_trash_shim() {
  ...
  cat >/tmp/NexisClaw-bin/trash <<'TRASH'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/NexisClaw-bin/trash
}
NexisClaw_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
...
NexisClaw_e2e_terminate_gateways() {
  local pid="${1:-}" _
...
NexisClaw_e2e_start_mock_openai() { MOCK_PORT="$1" node scripts/e2e/mock-openai-server.mjs >"$2" 2>&1 & printf '%s\n' "$!"; }
NexisClaw_e2e_wait_mock_openai() {
...
NexisClaw_e2e_start_gateway() { node "$1" gateway --port "$2" --bind loopback --allow-unconfigured >"$3" 2>&1 & printf '%s\n' "$!"; }
NexisClaw_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
NexisClaw_e2e_wait_gateway_ready() {
...
NexisClaw_e2e_probe_http_status() {
  node -e 'fetch(process.argv[1]).then(r=>process.exit(r.status===Number(process.argv[2])?0:1)).catch(()=>process.exit(1))' "$1" "${2:-200}"
}
NexisClaw_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
NexisClaw_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
NexisClaw_e2e_assert_log_not_contains() {
  ! grep -q "$2" "$1" || { echo "Unexpected log output: $2"; exit 1; }
}
NexisClaw_e2e_run_logged() {
  local label="$1" log_path="/tmp/NexisClaw-onboard-${1}.log"
  shift
