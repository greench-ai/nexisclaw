...
function usage() {
  return `Usage:
  node scripts/lib/NexisClaw-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/NexisClaw-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/NexisClaw-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "NexisClaw-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "NexisClaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `NexisClaw-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `NexisClaw-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    ...
    'export NEXISCLAW_STATE_DIR="$NEXISCLAW_TEST_STATE_HOME/.NexisClaw"',
...
export function renderShellFunction() {
  return `NexisClaw_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown NexisClaw test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    ...
    *)
      ...
      mkdir -p "$tmp_root"
      NEXISCLAW_TEST_STATE_HOME="$(mktemp -d "$tmp_root/NexisClaw-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$NEXISCLAW_TEST_STATE_HOME"
  export USERPROFILE="$NEXISCLAW_TEST_STATE_HOME"
