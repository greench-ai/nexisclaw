...
#
# Suggested cron: */30 * * * * /home/admin/NexisClaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send NexisClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., NexisClaw-alerts)
...
STATE_FILE="$HOME/.NexisClaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via NexisClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use NexisClaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via NexisClaw to $NOTIFY_PHONE..."
            NexisClaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! NexisClaw is down. Run: ssh l36 '~/NexisClaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
