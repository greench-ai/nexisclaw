...
const EXPECTED_REPOSITORY_URL = "https://github.com/NexisClaw/NexisClaw";
const OPTIONAL_LOCAL_EMBEDDING_RUNTIME_PACKAGE = "node-llama-cpp";
const FS_SAFE_PACKAGE = "@NexisClaw/fs-safe";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "NexisClaw") {
    errors.push(`package.json name must be "NexisClaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.NexisClaw !== "NexisClaw.mjs") {
    errors.push(
      `package.json bin.NexisClaw must be "NexisClaw.mjs"; found "${pkg.bin?.NexisClaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`NexisClaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `NexisClaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
