...
- NEXISCLAW_IOS_VERSION = exact canonical iOS version
- NEXISCLAW_MARKETING_VERSION = short iOS/App Store version
- NEXISCLAW_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    NEXISCLAW_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    NEXISCLAW_MARKETING_VERSION)
...
NEXISCLAW_IOS_VERSION = ${IOS_VERSION}
NEXISCLAW_MARKETING_VERSION = ${MARKETING_VERSION}
NEXISCLAW_BUILD_VERSION = ${BUILD_NUMBER}
EOF
