#!/usr/bin/env node
// Builds the NexisClaw package artifact used by Docker E2E.
...
async function newestNexisClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed NexisClaw tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building NexisClaw package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building NexisClaw Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing NexisClaw package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing NexisClaw package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestNexisClawTarball(outputDir, packOutput);
  ...
  console.error("==> Checking NexisClaw package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-NexisClaw-package-tarball.mjs"), tarball],
    sourceDir,
    { timeoutMs: 5 * 60 * 1000 },
  );
  console.error(
