...
const DEFAULT_OUTPUT_NAME = "NexisClaw-current.tgz";
export const NEXISCLAW_PACKAGE_SPEC_RE =
  /^NexisClaw@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-NexisClaw-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateNexisClawPackageSpec(spec) {
  if (!NEXISCLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be NexisClaw@alpha, NexisClaw@beta, NexisClaw@latest, or an exact NexisClaw release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an NexisClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `NexisClaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (/^NexisClaw-.*\.tgz$/u.test(trimmed)) {
        filename = trimmed;
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^NexisClaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no NexisClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-NexisClaw-for-docker.mjs",
        "--source-dir",
      ...
      validateNexisClawPackageSpec(options.packageSpec);
  ...
  console.error(`Checking NexisClaw package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-NexisClaw-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
    `NexisClaw package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
  ...
  if (pkg.name !== "NexisClaw") {
    throw new Error(`package candidate must be named "NexisClaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
