#!/bin/bash
# Setup NexisClaw Auth Management System
...
echo "=== NexisClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'NexisClaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  NexisClaw message: Send warning via NexisClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/NexisClaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/NexisClaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/NexisClaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now NexisClaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status NexisClaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
