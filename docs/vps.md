---
summary: "Run NexisClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for NexisClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the NexisClaw Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install NexisClaw on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep NexisClaw bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/NexisClaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
For the standard `NexisClaw onboard --install-daemon` path, edit the user unit:
...
`NexisClaw-gateway.service` via `sudo systemctl edit NexisClaw-gateway.service`.
