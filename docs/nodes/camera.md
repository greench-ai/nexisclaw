...
NexisClaw supports **camera capture** for agent workflows:
...
```bash
NexisClaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
NexisClaw nodes camera snap --node <id> --facing front
NexisClaw nodes camera clip --node <id> --duration 3000
NexisClaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`NexisClaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `NexisClaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
NexisClaw nodes camera list --node <id>            # list camera ids
NexisClaw nodes camera snap --node <id>            # prints MEDIA:<path>
NexisClaw nodes camera snap --node <id> --max-width 1280
NexisClaw nodes camera snap --node <id> --delay-ms 2000
NexisClaw nodes camera snap --node <id> --device-id <id>
NexisClaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
NexisClaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
NexisClaw nodes camera clip --node <id> --device-id <id>
NexisClaw nodes camera clip --node <id> --no-audio
```

Notes:

- `NexisClaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
NexisClaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
