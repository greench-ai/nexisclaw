...
## Config (`~/.NexisClaw/NexisClaw.json`)
...
- `consultThinkingLevel`: optional thinking level override for the full NexisClaw agent run behind realtime `NexisClaw_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `NexisClaw_agent_consult` calls.
...
- `realtime.instructions`: appends provider-facing system instructions to NexisClaw's built-in realtime prompt. Use it for voice style and tone; NexisClaw keeps the default `NexisClaw_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `NexisClaw_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `NexisClaw-mlx-tts` helper when present, or an executable on `PATH`. Set `NEXISCLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
