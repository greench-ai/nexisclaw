---
summary: "CLI reference for `NexisClaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
NexisClaw qr
NexisClaw qr --setup-code-only
NexisClaw qr --json
NexisClaw qr --remote
NexisClaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, NexisClaw requires either `gateway.remote.url` or
