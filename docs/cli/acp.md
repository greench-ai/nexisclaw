...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an NexisClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`NexisClaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to NexisClaw channel
conversations instead of hosting an ACP harness session, use
[`NexisClaw mcp serve`](/cli/mcp) instead.
...
- NexisClaw acts as an ACP server
- an IDE or ACP client connects to NexisClaw
- NexisClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where NexisClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to NexisClaw: use `NexisClaw acp`
- NexisClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include NexisClaw parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the NexisClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
NexisClaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
NexisClaw acp --url wss://gateway-host:18789 --token-file ~/.NexisClaw/gateway.token

# Attach to an existing session key
NexisClaw acp --session agent:main:main

# Attach by label (must already exist)
NexisClaw acp --session-label "support inbox"

# Reset the session key before the first prompt
NexisClaw acp --session agent:main:main --reset-session
```
...
```bash
NexisClaw acp client

# Point the spawned bridge at a remote Gateway
NexisClaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.NexisClaw/gateway.token

# Override the server command (default: NexisClaw)
NexisClaw acp client --server "node" --server-args NexisClaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run NexisClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`NexisClaw acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `NexisClaw gateway call sessions.list` as the only ACP proof. That
...
it to drive an NexisClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `NexisClaw acp` over stdio.

Example config (persisted):

```bash
NexisClaw config set gateway.remote.url wss://gateway-host:18789
NexisClaw config set gateway.remote.token <token>
```
...
```bash
NexisClaw acp --session agent:main:main
NexisClaw acp --session agent:design:main
NexisClaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see NexisClaw plugin tools or selected
...
[NexisClaw tools MCP bridge](/tools/acp-agents-setup#NexisClaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
NexisClaw bot over ACP, use `acpx` with its built-in `NexisClaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx NexisClaw` at `NexisClaw acp`.
3. Target the NexisClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default NexisClaw ACP session
acpx NexisClaw exec "Summarize the active NexisClaw session state."

# Persistent named session for follow-up turns
acpx NexisClaw sessions ensure --name codex-bridge
acpx NexisClaw -s codex-bridge --cwd /path/to/repo \
  "Ask my NexisClaw work agent for recent context relevant to this repo."
```

If you want `acpx NexisClaw` to target a specific Gateway and session key every
time, override the `NexisClaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "NexisClaw": {
      "command": "env NEXISCLAW_HIDE_BANNER=1 NEXISCLAW_SUPPRESS_NOTES=1 NexisClaw acp --url ws://127.0.0.1:18789 --token-file ~/.NexisClaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local NexisClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env NEXISCLAW_HIDE_BANNER=1 NEXISCLAW_SUPPRESS_NOTES=1 node NexisClaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an NexisClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select "NexisClaw ACP" to start a thread.
...
- `NexisClaw acp client` sets `NEXISCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `NexisClaw`).
