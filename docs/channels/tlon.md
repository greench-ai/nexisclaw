...
Tlon is a decentralized messenger built on Urbit. NexisClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current NexisClaw releases, so normal packaged
...
```bash
NexisClaw plugins install @NexisClaw/tlon
```
...
```bash
NexisClaw plugins install ./path/to/local/tlon-plugin
```
...
By default, NexisClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `NexisClaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, NexisClaw replies in-thread.
