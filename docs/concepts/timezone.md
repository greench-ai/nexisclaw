---
summary: "Where timezones show up in NexisClaw — envelopes, tool payloads, system prompt"
read_when:
...
NexisClaw standardizes timestamps so the model sees a **single reference time** instead of a mix of provider-local clocks. There are three surfaces where timezones show up, each with its own purpose:
...
If `userTimezone` is unset, NexisClaw resolves the host timezone at runtime (no config write). `agents.defaults.timeFormat` (`auto` | `12` | `24`) controls 12h/24h rendering in envelopes and downstream surfaces, not in the system prompt section.
