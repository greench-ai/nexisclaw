...
alongside NexisClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, NexisClaw falls back to the
...
NexisClaw creates a self-contained QMD home under
`~/.NexisClaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- NexisClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so NexisClaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, NexisClaw retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, NexisClaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, NexisClaw falls back to the builtin SQLite engine.
  ...
  `NexisClaw memory status` and one-shot CLI probes still recheck QMD directly.
...
NexisClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, NexisClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, NexisClaw searches all
...
Older QMD builds only accept one collection filter. When NexisClaw detects one
...
process, so you can tune QMD globally without adding new NexisClaw config:
...
collection under `~/.NexisClaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, NexisClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If NexisClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but NexisClaw still reports
...
with `NexisClaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs NexisClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. NexisClaw uses
...
`memory.qmd.searchMode = "search"`. NexisClaw treats that mode as lexical-only,
...
the next startup reconciliation. NexisClaw recreates stale managed collections
...
NexisClaw's builtin symlink rules. Keep temporary monorepo checkouts under
