...
- `/usage cost` in chats: local cost summary aggregated from NexisClaw session logs.
- CLI: `NexisClaw status --usage` prints a full per-provider breakdown.
- CLI: `NexisClaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. NexisClaw treats
  ...
  quota, so NexisClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, NexisClaw prefers the
...
can supply plugin-specific usage auth logic; otherwise NexisClaw falls back to
