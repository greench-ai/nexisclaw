---
summary: "How NexisClaw rotates auth profiles and falls back across models"
read_when:
...
NexisClaw handles failures in two stages:
...
For a normal text run, NexisClaw evaluates candidates in this order:
...
NexisClaw separates the selected provider/model from why it was selected. That source controls whether the fallback chain is allowed:
...
- **User session override**: `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` write `modelOverrideSource: "user"`. That is an exact session selection. If the selected provider/model fails before producing a reply, NexisClaw reports the failure instead of answering from an unrelated configured fallback.
- **Legacy session override**: older session entries may have `modelOverride` without `modelOverrideSource`. NexisClaw treats those as user overrides so an explicit old selection is not silently converted into fallback behavior.
...
NexisClaw uses **auth profiles** for both API keys and OAuth tokens.

- Secrets live in `~/.NexisClaw/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.NexisClaw/agent/auth-profiles.json`).
- Runtime auth-routing state lives in `~/.NexisClaw/agents/<agentId>/agent/auth-state.json`.
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.NexisClaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.NexisClaw/agents/<agentId>/agent/auth-profiles.json` under `profiles`.

## Rotation order

When a provider has multiple profiles, NexisClaw chooses an order like this:
...
If no explicit order is configured, NexisClaw uses a round-robin order:
...
NexisClaw **pins the chosen auth profile per session** to keep provider caches warm. It does **not** rotate on every request. The pinned profile is reused until:
...
<Note>
Auto-pinned profiles (selected by the session router) are treated as a **preference**: they are tried first, but NexisClaw may rotate to another profile on rate limits/timeouts. When the original profile becomes available again, new runs can prefer it again without changing the selected model or runtime. User-pinned profiles stay locked to that profile; if it fails and model fallbacks are configured, NexisClaw moves to the next model instead of switching profiles.
</Note>
...
NexisClaw records the exact reset time when Codex provides one, tries the next
...
When a profile fails due to auth/rate-limit errors (or a timeout that looks like rate limiting), NexisClaw marks it in cooldown and moves to the next profile.
...
Billing/credit failures (for example "insufficient credits" / "credit balance too low") are treated as failover-worthy, but they're usually not transient. Instead of a short cooldown, NexisClaw marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

<Note>
Not every billing-shaped response is `402`, and not every HTTP `402` lands here. NexisClaw keeps explicit billing text in the billing lane even when a provider returns `401` or `403` instead, but provider-specific matchers stay scoped to the provider that owns them (for example OpenRouter `403 Key limit exceeded`).
...
If all profiles for a provider fail, NexisClaw moves to the next model in `agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and timeouts that exhausted profile rotation (other errors do not advance fallback). Provider errors that do not expose enough detail are still labeled precisely in fallback state: `empty_response` means the provider returned no usable message or status, `no_error_details` means the provider explicitly returned `Unknown error (no error details in response)`, and `unclassified` means NexisClaw preserved the raw preview but no classifier matched it yet.

Overloaded and rate-limit errors are handled more aggressively than billing cooldowns. By default, NexisClaw allows one same-provider auth-profile retry, then switches to the next configured model fallback without waiting. Provider-busy signals such as `ModelNotReadyException` land in that overloaded bucket. Tune this with `auth.cooldowns.overloadedProfileRotations`, `auth.cooldowns.overloadedBackoffMs`, and `auth.cooldowns.rateLimitedProfileRotations`.

When a run starts from the configured default primary, a cron job primary, an agent primary with explicit fallbacks, or an auto-selected fallback override, NexisClaw can walk the matching configured fallback chain. Agent primaries without explicit fallbacks and explicit user selections (for example `/model ollama/qwen3.5:27b`, the model picker, `sessions.patch`, or one-off CLI provider/model overrides) are strict: if that provider/model is unreachable or fails before producing a reply, NexisClaw reports the failure instead of answering from an unrelated fallback.

### Candidate chain rules

NexisClaw builds the candidate list from the currently requested `provider/model` plus configured fallbacks.
...
<AccordionGroup>
  <Accordion title="Rules">
    ...
    - If the current run is already on a configured fallback in the same provider family, NexisClaw keeps using the full configured chain.
...
When every auth profile for a provider is already in cooldown, NexisClaw does not automatically skip that provider forever. It makes a per-candidate decision:
...
- Auto fallback overrides remain selected on subsequent turns so NexisClaw does not probe a known-bad primary on every message. `/new`, `/reset`, and `sessions.reset` clear auto-sourced overrides and return the session to the configured default.
...
- If a live-switch error points at a later candidate in the active fallback chain, NexisClaw jumps directly to that selected model instead of walking unrelated candidates first.
...
When every candidate fails, NexisClaw throws `FallbackSummaryError`. The outer reply runner can use that to build a more specific message such as "all models are temporarily rate-limited" and include the soonest cooldown expiry when one is known.
...
- if the remaining block is a matching model-scoped rate limit, NexisClaw reports the last matching expiry that still blocks that model
