...
read_when:
  ...
  - Explaining how NexisClaw updates one channel message while work is in progress
...
When progress drafts are enabled, NexisClaw creates one visible work-in-progress
...
That is usually enough. NexisClaw will pick an automatic one-word label, wait
...
NexisClaw sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from NexisClaw's built-in
...
NexisClaw uses the same formatter for progress drafts and `/verbose`:
...
NexisClaw truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, NexisClaw still suppresses the older standalone
...
When the final answer is ready, NexisClaw tries to keep the chat clean:

- If the draft can safely become the final answer, NexisClaw edits it in place.
- If the channel uses native progress streaming, NexisClaw finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, NexisClaw sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, NexisClaw keeps the
