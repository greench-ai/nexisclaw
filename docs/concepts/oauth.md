---
summary: "OAuth in NexisClaw: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand NexisClaw OAuth end-to-end
...
NexisClaw also supports **provider plugins** that ship their own OAuth or API-key
flows. Run them via:
...
- you log in via NexisClaw _and_ via Claude Code / Codex CLI → one of them randomly gets "logged out" later

To reduce that, NexisClaw treats `auth-profiles.json` as a **token sink**:

- the runtime reads credentials from **one place**
- we can keep multiple profiles and route them deterministically
- external CLI reuse is provider-specific: Codex CLI can bootstrap an empty
  `openai-codex:default` profile, but once NexisClaw has a local OAuth profile,
...
- Auth profiles (OAuth + API keys + optional value-level refs): `~/.NexisClaw/agents/<agentId>/agent/auth-profiles.json`
- Legacy compatibility file: `~/.NexisClaw/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.NexisClaw/credentials/oauth.json` (imported into `auth-profiles.json` on first use)
...
When a secondary agent has no local auth profile, NexisClaw uses read-through
...
Claude subscription limits, and Anthropic staff told us NexisClaw-style Claude
CLI usage is allowed again. NexisClaw therefore treats Claude CLI reuse and
...
If you want other subscription-style options in NexisClaw, see [OpenAI
Codex](/providers/openai), [Qwen Cloud Coding
Plan](/providers/qwen), [MiniMax Coding Plan](/providers/minimax),
and [Z.AI / GLM Coding Plan](/providers/glm).
</Warning>

NexisClaw also exposes Anthropic setup-token as a supported token-auth path, but it now prefers Claude CLI reuse and `claude -p` when available.

## Anthropic Claude CLI migration

NexisClaw supports Anthropic Claude CLI reuse again. If you already have a local
Claude login on the host, onboarding/configure can reuse it directly.

## OAuth exchange (how login works)

NexisClaw's interactive login flows are implemented in `@earendil-works/pi-ai` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from NexisClaw
2. NexisClaw stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including NexisClaw workflows.
...
Wizard path is `NexisClaw onboard` → auth choice `openai-codex`.
...
- exception: some external CLI credentials stay externally managed; NexisClaw
  ...
  `openai-codex:default` profile, then NexisClaw-owned refreshes keep the local
  profile canonical.
...
- `NexisClaw channels list --json` (shows `auth[]`)
