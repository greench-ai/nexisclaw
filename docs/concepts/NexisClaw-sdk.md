---
summary: "Public NexisClaw App SDK for external apps, scripts, dashboards, CI jobs, and IDE extensions"
title: "NexisClaw App SDK"
sidebarTitle: "App SDK"
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to NexisClaw
...
The **NexisClaw App SDK** is the public client API for apps outside the
NexisClaw process. Use `@NexisClaw/sdk` when a script, dashboard, CI job, IDE
...
<Note>
  ...
  `@NexisClaw/sdk` talks to the Gateway from outside NexisClaw.
  `NexisClaw/plugin-sdk/*` is only for plugins that run inside NexisClaw and
  register providers, channels, tools, hooks, or trusted runtimes.
</Note>

## What ships today

`@NexisClaw/sdk` ships with:

| Surface                   | Status  | What it does                                                                      |
| ------------------------- | ------- | --------------------------------------------------------------------------------- |
| `NexisClaw`                | Ready   | Main client entry point. Owns transport, connection, requests, and events.        |
...
`AgentRunParams`, `RunResult`, `RunStatus`, `NexisClawEvent`,
`NexisClawEventType`, `GatewayEvent`, `NexisClawTransport`,
...
```typescript
import { NexisClaw } from "@NexisClaw/sdk";
...
`new NexisClaw({ gateway: "ws://..." })` is equivalent to `url`. The
...
For tests, pass an object that implements `NexisClawTransport`:
...
The SDK normalizes raw Gateway events into a stable `NexisClawEvent` envelope:
...
Task helpers use the durable task ledger that also backs `NexisClaw tasks`:
...
Use the App SDK when code lives outside NexisClaw:
...
Use the Plugin SDK when code runs inside NexisClaw:
...
App SDK code should import from `@NexisClaw/sdk`. Plugin code should import from
documented `NexisClaw/plugin-sdk/*` subpaths. Do not mix the two contracts.

## Related

- [NexisClaw App SDK API design](/reference/NexisClaw-sdk-api-design)
