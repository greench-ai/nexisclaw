---
summary: "How NexisClaw builds prompt context and reports token usage + costs"
read_when:
...
NexisClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

NexisClaw assembles its own system prompt on every run. It includes:
...
For images, NexisClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from NexisClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `NexisClaw status --usage` and `NexisClaw channels list` show
...
available; otherwise NexisClaw falls back to matching OAuth/API-key credentials
...
NexisClaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, NexisClaw shows tokens only. OAuth tokens
never show dollar cost.

After sidecars and channels reach the Gateway ready path, NexisClaw starts an
...
Provider prompt caching only applies within the cache TTL window. NexisClaw can
...
Anthropic's 1M context window is currently beta-gated. NexisClaw can inject the
...
NexisClaw skips the `context-1m-*` beta header because Anthropic currently
