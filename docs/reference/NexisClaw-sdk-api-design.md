---
summary: "Reference design for the public NexisClaw App SDK API, event taxonomy, artifacts, approvals, and package structure"
title: "NexisClaw App SDK API design"
sidebarTitle: "App SDK API design"
read_when:
  - You are implementing the proposed public NexisClaw app SDK
  - You need the draft namespace, event, result, artifact, approval, or security contract for the app SDK
  - You are comparing Gateway protocol resources with the high-level NexisClaw App SDK wrapper
---

This page is the detailed API reference design for the public
[NexisClaw App SDK](/concepts/NexisClaw-sdk). It is intentionally separate from
the [Plugin SDK](/plugins/sdk-overview).

<Note>
  `@NexisClaw/sdk` is the external app/client package for talking to the
  Gateway. `NexisClaw/plugin-sdk/*` is the in-process plugin authoring contract.
...
2. A high-level ergonomic wrapper with `NexisClaw`, `Agent`, `Session`, `Run`,
...
- source: NexisClaw, MCP, plugin, channel, runtime, or app
- schema summary
- approval policy
...
| Package                 | Purpose                                                       |
| ----------------------- | ------------------------------------------------------------- |
| `@NexisClaw/sdk`         | Public high-level SDK and generated low-level Gateway client. |
| `@NexisClaw/sdk-react`   | Optional React hooks for dashboards and app builders.         |
| `@NexisClaw/sdk-testing` | Test helpers and fake Gateway server for app integrations.    |

The repo already has `NexisClaw/plugin-sdk/*` for plugins. Keep that namespace
...
4. High-level `NexisClaw`, `Agent`, `Session`, `Run`, `Task`, and `Artifact`
   wrappers.
...
- [NexisClaw App SDK](/concepts/NexisClaw-sdk)
