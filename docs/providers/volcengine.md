...
read_when:
  - You want to use Volcano Engine or Doubao models with NexisClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    NexisClaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    NexisClaw models list --provider volcengine
    NexisClaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `NexisClaw.json`:
...
For voice-note targets, NexisClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `NexisClaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, NexisClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.NexisClaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running NexisClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about NexisClaw setup.
  </Card>
</CardGroup>
