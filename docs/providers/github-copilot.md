---
summary: "Sign in to GitHub Copilot from NexisClaw using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `NexisClaw models auth login-github-copilot` flow
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. NexisClaw can use Copilot as a model
provider in two different ways.

## Two ways to use Copilot in NexisClaw

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when NexisClaw runs. This is the **default** and simplest path
    because it does not require VS Code.

    <Steps>
      <Step title="Run the login command">
        ```bash
        NexisClaw models auth login-github-copilot
        ```
      ...
      <Step title="Set a default model">
        ```bash
        NexisClaw models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. NexisClaw talks to
...
```bash
# Skip confirmation
NexisClaw models auth login-github-copilot --yes

# Login and set the default model in one step
NexisClaw models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

If you already have a GitHub OAuth access token for Copilot, import it during
headless setup with `NexisClaw onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    NexisClaw refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an NexisClaw
  ...
  <Accordion title="Transport selection">
    ...
    o-series, and Gemini models keep the OpenAI Responses transport. NexisClaw
    selects the correct transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    NexisClaw sends Copilot IDE-style request headers on Copilot transports,
  ...
  <Accordion title="Environment variable resolution order">
    NexisClaw resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, NexisClaw uses the highest-priority one.
    The device-login flow (`NexisClaw models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    for a Copilot API token when NexisClaw runs. You do not need to manage the
    token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, NexisClaw can use it for embeddings without a separate API key.
...
providers. If a GitHub token is available, NexisClaw discovers available
...
1. NexisClaw resolves your GitHub token (from env vars or auth profile).
...
available, NexisClaw skips Copilot and tries the next provider.
