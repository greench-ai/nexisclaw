---
summary: "Use Synthetic's Anthropic-compatible API in NexisClaw"
read_when:
...
NexisClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    NexisClaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
NexisClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`NexisClaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that NexisClaw appends `/v1` automatically.
