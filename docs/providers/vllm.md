---
summary: "Run NexisClaw with vLLM (OpenAI-compatible local server)"
read_when:
  - You want to run NexisClaw against a local vLLM server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "vLLM"
---

vLLM can serve open-source (and some custom) models via an **OpenAI-compatible** HTTP API. NexisClaw connects to vLLM using the `openai-completions` API.

NexisClaw can also **auto-discover** available models from vLLM when you opt in with `VLLM_API_KEY` (any value works if your server does not enforce auth) and you do not define an explicit `models.providers.vllm` entry.

NexisClaw treats `vllm` as a local OpenAI-compatible provider that supports
...
<Steps>
  ...
  <Step title="Verify the model is available">
    ```bash
    NexisClaw models list --provider vllm
    ```
  </Step>
</Steps>
...
When `VLLM_API_KEY` is set (or an auth profile exists) and you **do not** define `models.providers.vllm`, NexisClaw queries:
...
<AccordionGroup>
  <Accordion title="Proxy-style behavior">
    ...
    | Hidden NexisClaw attribution headers | Not injected on custom base URLs |
  ...
  <Accordion title="Qwen thinking controls">
    ...
    server expects Qwen chat-template kwargs. NexisClaw maps `/think off` to:
  ...
  <Accordion title="Nemotron 3 thinking controls">
    ...
    returned as hidden reasoning or visible answer text. When an NexisClaw session
  ...
  <Accordion title="Qwen tool calls appear as text">
    ...
    - vLLM returns an empty `tool_calls` array when NexisClaw sends
      `tool_choice: "auto"`
    ...
    ```bash
    NexisClaw config set agents.defaults.models '{"vllm/Qwen-Qwen2.5-Coder-32B-Instruct":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Auth errors on requests">
    ...
    <Tip>
    If your vLLM server does not enforce auth, any non-empty value for `VLLM_API_KEY` works as an opt-in signal for NexisClaw.
    </Tip>

  </Accordion>

  <Accordion title="No models discovered">
    Auto-discovery requires `VLLM_API_KEY` to be set **and** no explicit `models.providers.vllm` config entry. If you have defined the provider manually, NexisClaw skips discovery and uses only your declared models.
  </Accordion>
  ...
  <Accordion title="Tools render as raw text">
    ...
    - confirm the exact model id with `NexisClaw models list --provider vllm`
