...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with NexisClaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    NexisClaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    NexisClaw models list --provider deepseek
    ```
    ...
    ```bash
    NexisClaw models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. NexisClaw also replays
...
`reasoning_content` on follow-up requests. NexisClaw handles this inside the
...
DeepSeek `reasoning_content`. NexisClaw fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in NexisClaw (including the UI **None** selection),
NexisClaw sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
