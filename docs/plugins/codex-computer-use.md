---
summary: "Set up Codex Computer Use for Codex-mode NexisClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode NexisClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. NexisClaw
...
Use this page when NexisClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## NexisClaw.app and Peekaboo

NexisClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want NexisClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode NexisClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an NexisClaw node and exposes mobile
...
NexisClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through NexisClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the NexisClaw command:

```bash
cua-driver mcp-config --client NexisClaw
```

or register the stdio server yourself:

```bash
NexisClaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal NexisClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. NexisClaw
...
With this config, NexisClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, NexisClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, NexisClaw also tries to
...
plugin command surface is available. These are NexisClaw chat/runtime commands,
not `NexisClaw codex ...` CLI subcommands:
...
NexisClaw uses the same app-server API that Codex itself exposes. The
...
During install, NexisClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, NexisClaw prefers
...
`computer-use` is registered, NexisClaw tries to add the standard bundled
...
## What NexisClaw checks

NexisClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If NexisClaw says Computer Use
...
NexisClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh NexisClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook could not reach an active NexisClaw relay through the local bridge or
Gateway fallback. Start a fresh NexisClaw session with `/new` or `/reset`. If it
