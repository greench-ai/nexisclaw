...
At startup, NexisClaw does roughly this:
...
The manifest is the control-plane source of truth. NexisClaw uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, NexisClaw
...
NexisClaw does not cache plugin discovery results or direct manifest registry
...
NexisClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, NexisClaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | NexisClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on NexisClaw's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of NexisClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as NexisClaw adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while NexisClaw owns the
...
- NexisClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "NexisClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-NexisClaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-NexisClaw-scopes` only when the header is explicitly present
  - if `x-NexisClaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-NexisClaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `NexisClaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `NexisClaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `NexisClaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `NexisClaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `NexisClaw/plugin-sdk/config-schema` | Root `NexisClaw.json` Zod schema (`NexisClawSchema`) |
...
<Info>
`NexisClaw/plugin-sdk/channel-runtime`, `NexisClaw/plugin-sdk/config-runtime`,
and `NexisClaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `NexisClaw/plugin-sdk/*` subpaths. Never
...
`NexisClaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape NexisClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to NexisClaw's
...
- if both `catalog` and `discovery` are registered, NexisClaw uses `catalog`
...
- Read-only command paths such as `NexisClaw status`, `NexisClaw status --all`,
  `NexisClaw channels status`, `NexisClaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `NexisClaw.extensions`:
...
Security guardrail: every `NexisClaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `NexisClaw plugins install` installs plugin dependencies with a
...
Optional: `NexisClaw.setupEntry` can point at a lightweight setup-only module.
When NexisClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `NexisClaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let NexisClaw load the
...
Channel plugins can advertise setup/discovery metadata via `NexisClaw.channel` and
install hints via `NexisClaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@NexisClaw/nextcloud-talk",
  "NexisClaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@NexisClaw/nextcloud-talk",
...
Useful `NexisClaw.channel` fields beyond the minimal example:
...
NexisClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.NexisClaw/mpm/plugins.json`
- `~/.NexisClaw/mpm/catalog.json`
- `~/.NexisClaw/plugins/catalog.json`

Or point `NEXISCLAW_PLUGIN_CATALOG_PATHS` (or `NEXISCLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "NexisClaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `NexisClaw.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "NexisClaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `NexisClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how NexisClaw stays opinionated without becoming hardcoded to one
