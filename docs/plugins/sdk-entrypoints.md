...
when NexisClaw loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, NexisClaw will use a
...
**Import:** `NexisClaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `NexisClawPluginConfigSchema \| () => NexisClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: NexisClawPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `NexisClaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- NexisClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `NexisClaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: NexisClawPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: NexisClawPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. NexisClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and NexisClaw
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `NexisClaw nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; NexisClaw resolves it to the
...
NexisClaw loads this instead of the full entry when a channel is disabled,
...
- `NexisClaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `NexisClaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `NexisClaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`NexisClaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so NexisClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want NexisClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; NexisClaw rejects descriptor names outside
...
NexisClaw classifies loaded plugins by their registration behavior:
...
Use `NexisClaw plugins inspect <id>` to see a plugin's shape.
