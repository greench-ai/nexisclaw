...
the native model loop, and NexisClaw adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

NexisClaw still owns channel routing, session files, visible message delivery,
NexisClaw dynamic tools, approvals, media delivery, and a transcript mirror.
...
When an NexisClaw session is attached to an existing Codex thread, the next turn
...
to the NexisClaw `message` tool if the deployment has not explicitly configured
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable NexisClaw
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| NexisClaw plugin hooks                 | NexisClaw                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | NexisClaw bundled plugins | Per-turn adapter behavior around NexisClaw dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

NexisClaw does not use project or global Codex `hooks.json` files to route
NexisClaw plugin behavior. For the supported native tool and permission bridge,
NexisClaw injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and NexisClaw's approval bridge handle real
...
Codex-level controls. They are not exposed as NexisClaw plugin hooks in the v1
contract.

For NexisClaw dynamic tools, NexisClaw executes the tool after Codex asks for the
call, so NexisClaw fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
NexisClaw can mirror selected events, but it cannot rewrite the native Codex
...
notifications and NexisClaw adapter state, not native Codex hook commands.
NexisClaw's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke NexisClaw plugin hooks.
...
| NexisClaw channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                           |
| NexisClaw dynamic tools                        | Supported                                                                        | Codex asks NexisClaw to execute these tools, so NexisClaw stays in the execution path.                                                                                                                       |
| Prompt and context plugins                    | Supported                                                                        | NexisClaw builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                                                                                           |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around NexisClaw-owned dynamic tools.                                                                                                 |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through NexisClaw after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review. |
| App-server trajectory capture                 | Supported                                                                        | NexisClaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                           |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but NexisClaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. NexisClaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms NexisClaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | NexisClaw observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current NexisClaw compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | NexisClaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, NexisClaw only returns explicit allow or deny decisions
...
NexisClaw remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through NexisClaw's plugin
...
default `messages.queue.mode: "steer"`, NexisClaw batches queued chat messages
...
case, NexisClaw uses the follow-up queue when the selected mode allows fallback.
...
harness, NexisClaw also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, NexisClaw session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, NexisClaw does not print those Codex ids and
...
delegated to Codex app-server. NexisClaw keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, NexisClaw only
...
NexisClaw is writing an NexisClaw-owned session transcript tool result.

## Media and delivery

NexisClaw continues to own media delivery and media provider selection. Image,
...
through the normal NexisClaw delivery path. Media generation does not require PI.
When Codex emits a native image-generation item with a `savedPath`, NexisClaw
