...
service via the external `NexisClaw` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.NexisClaw.gateway`
  (or `ai.NexisClaw.<profile>` when using `--profile`/`NEXISCLAW_PROFILE`; legacy `com.NexisClaw.*` is supported).
