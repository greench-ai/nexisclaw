...
- Same path: run the app from a fixed location (for NexisClaw, `dist/NexisClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.NexisClaw.mac
sudo tccutil reset ScreenCapture ai.NexisClaw.mac
...
Workaround: move files into the NexisClaw workspace (`~/.NexisClaw/workspace`) if you want to avoid per-folder grants.
