...
2. `npm i -g NexisClaw@latest`
3. `NexisClaw onboard --install-daemon`
...
NexisClaw installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `NexisClaw gateway install` and
`NexisClaw onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/NexisClaw-gateway[-<profile>].service`:

```
[Unit]
Description=NexisClaw Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. NexisClaw therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, NexisClaw starts the child through a short
...
- NexisClaw-launched browser/Chrome processes.
