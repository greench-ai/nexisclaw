...
NexisClaw supports OAuth and API keys for model providers. For always-on gateway
...
setup, but NexisClaw also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `NexisClaw gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.NexisClaw/.env` so the daemon can read it:
...
API keys for daemon use: `NexisClaw onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.NexisClaw/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in NexisClaw as a supported token
path. Anthropic staff has since told us that NexisClaw-style Claude CLI usage is
allowed again, so NexisClaw treats Claude CLI reuse and `claude -p` usage as
...
2. Tell NexisClaw to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching NexisClaw auth profile.
...
```bash
NexisClaw models auth paste-token --provider openrouter
```
...
NexisClaw expects the canonical `version` + `profiles` shape at runtime. If an older install still has a flat file such as `{ "openrouter": { "apiKey": "..." } }`, run `NexisClaw doctor --fix` to rewrite it as an `openrouter:default` API-key profile; doctor keeps a `.legacy-flat.*.bak` copy beside the original. Endpoint details such as `baseUrl`, `api`, model ids, headers, and timeouts belong under `models.providers.<id>` in `NexisClaw.json` or `models.json`, not in `auth-profiles.json`.

External auth routes such as Bedrock `auth: "aws-sdk"` are also not credentials. If you want a named Bedrock route, put `auth.profiles.<id>.mode: "aws-sdk"` in `NexisClaw.json`; do not write `type: "aws-sdk"` into `auth-profiles.json`. `NexisClaw doctor --fix` moves legacy AWS SDK markers from the credential store into config metadata.
...
- If auth exists but NexisClaw cannot resolve a probeable model candidate for
...
- Anthropic staff told us this NexisClaw integration path is allowed again.
- NexisClaw therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- NexisClaw retries with the next key only for rate-limit errors (for example
...
When you debug order issues, `NexisClaw models status --probe` shows omitted
...
Run `NexisClaw models status` to confirm which profile is expiring. If an
