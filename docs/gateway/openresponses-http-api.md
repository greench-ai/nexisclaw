...
NexisClaw's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`NexisClaw agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-NexisClaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-NexisClaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "NexisClaw"`, `model: "NexisClaw/default"`, `model: "NexisClaw/<agentId>"`, or `x-NexisClaw-agent-id`
- use `x-NexisClaw-model` when you want to override the selected agent's backend model
- use `x-NexisClaw-session-key` for explicit session routing
- use `x-NexisClaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: NexisClaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
NexisClaw normalizes common OpenAI-style aliases before those counters reach
