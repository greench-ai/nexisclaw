...
CLI onboarding is the **recommended** way to set up NexisClaw on macOS,
...
`NexisClaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`NexisClaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.NexisClaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `NexisClaw doctor` first.
</Note>
...
Use `NexisClaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.NexisClaw/workspace-<agentId>`.
...
- CLI command reference: [`NexisClaw onboard`](/cli/onboard)
