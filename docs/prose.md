---
summary: "OpenProse: .prose workflows, slash commands, and state in NexisClaw"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In NexisClaw it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
NexisClaw plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `NexisClaw plugins install ./path/to/local/open-prose-plugin`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses NexisClaw tools under the hood.
...
## NexisClaw runtime mapping

OpenProse programs map to NexisClaw primitives:

| OpenProse concept         | NexisClaw tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use NexisClaw tool allowlists and approval gates to control side effects.
