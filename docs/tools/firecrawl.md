...
NexisClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `NexisClaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
- Legacy `tools.web.fetch.firecrawl.*` config is auto-migrated by `NexisClaw doctor --fix`.
...
when you run Firecrawl yourself. NexisClaw accepts `http://` only for loopback,
...
NexisClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
...
The selection knob is `tools.web.fetch.provider`. If you omit it, NexisClaw
