...
read_when:
  ...
  - Debugging why NexisClaw is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (NexisClaw-managed)"
---

NexisClaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `NexisClaw` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **NexisClaw** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`NexisClaw`, `work`, `remote`, ...).
...
NexisClaw browser --browser-profile NexisClaw doctor --deep
NexisClaw browser --browser-profile NexisClaw status
...
If `NexisClaw browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `NexisClaw browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `NexisClaw browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `NexisClaw` vs `user`

- `NexisClaw`: managed, isolated browser (no extension required).
- `user`: built-in Chrome MCP attach profile for your **real signed-in Chrome**
  session.

For agent browser tool calls:

- Default: use the isolated `NexisClaw` browser.
...
Set `browser.defaultProfile: "NexisClaw"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.NexisClaw/NexisClaw.json`.
...
- Local `NexisClaw` profiles auto-assign `cdpPort`/`cdpUrl`; set those only for remote CDP. `cdpUrl` defaults to the managed local CDP port when unset.
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, NexisClaw pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the NexisClaw-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
...
- `POST /start?headless=true` and `NexisClaw browser start --headless` request a
  ...
  remote CDP profiles reject the override because NexisClaw does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `NexisClaw browser status --json`
...
- Default profile is `NexisClaw` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
NexisClaw uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that NexisClaw
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, NexisClaw will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local NexisClaw-managed browser profile.
- `headless` only affects local managed profiles that NexisClaw launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `NexisClaw browser stop` stops the browser process that
  NexisClaw launched
- attach-only and remote CDP profiles: `NexisClaw browser stop` closes the active
  ...
  though no browser process was launched by NexisClaw
...
NexisClaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, NexisClaw can
...
- If you set `nodeHost.browserProxy.allowProfiles`, NexisClaw treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. NexisClaw can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let NexisClaw
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and NexisClaw runs on the host, treat
...
NexisClaw process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-NexisClaw WebSocket base, such
...
an address NexisClaw cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, NexisClaw treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by NexisClaw.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). NexisClaw accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  NexisClaw calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. NexisClaw connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). NexisClaw tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise NexisClaw
  ...
  accepts it, NexisClaw falls back to that root as well. This lets a bare `ws://`
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-NexisClaw-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, NexisClaw
  ...
  restarts.
- NexisClaw does **not** auto-generate that token when `gateway.auth.mode` is
...
NexisClaw supports multiple named profiles (routing configs). Profiles can be:

- **NexisClaw-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `NexisClaw` profile is auto-created if missing.
...
NexisClaw can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when NexisClaw attaches.
...
```bash
NexisClaw browser --browser-profile user start
NexisClaw browser --browser-profile user status
NexisClaw browser --browser-profile user tabs
NexisClaw browser --browser-profile user snapshot --format ai
```
...
- `NexisClaw doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `NexisClaw` profile because it can
  act inside your signed-in browser session.
- NexisClaw does not launch the browser for this driver; it only attaches.
- NexisClaw uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, NexisClaw skips
...
Compared to the managed `NexisClaw` profile, existing-session drivers are more constrained:
...
When launching locally, NexisClaw picks the first available:
...
control API** plus a matching `NexisClaw browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means NexisClaw cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "NexisClaw" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by NexisClaw` when a
...
- For the local loopback `NexisClaw` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for NexisClaw's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (NexisClaw, chrome, or remote CDP).
