...
NexisClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, NexisClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `NexisClaw configure --section web`. It stores the key in
`~/.NexisClaw/NexisClaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.NexisClaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
