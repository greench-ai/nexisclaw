...
NexisClaw supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.NexisClaw/.env`.
...
`pd`, `pw`, `pm`, and `py`. NexisClaw converts these values, or an explicit
