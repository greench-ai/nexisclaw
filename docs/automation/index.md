...
read_when:
  - Deciding how to automate work with NexisClaw
  - Choosing between heartbeat, cron, commitments, hooks, and standing orders
  - Looking for the right automation entry point
title: "Automation"
---

NexisClaw runs work in the background through tasks, scheduled jobs, inferred
...
| Audit what ran and when                 | Background Tasks       | `NexisClaw tasks list` and `NexisClaw tasks audit` |
...
The background task ledger tracks all detached work: ACP runs, subagent spawns, isolated cron executions, and CLI operations. Tasks are records, not schedulers. Use `NexisClaw tasks list` and `NexisClaw tasks audit` to inspect them.

See [Background Tasks](/automation/tasks).

### Inferred commitments

Commitments are opt-in, short-lived follow-up memories. NexisClaw infers them
...
Task Flow is the flow orchestration substrate above background tasks. It manages durable multi-step flows with managed and mirrored sync modes, revision tracking, and `NexisClaw tasks flow list|show|cancel` for inspection.
...
with `NexisClaw hooks`. For in-process tool-call interception, use
