...
Explicit copy flows, such as `NexisClaw agents add`, use this portability policy:
...
has such a marker, `NexisClaw doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but NexisClaw cannot resolve a probeable model
