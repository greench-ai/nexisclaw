...
Security work is shared across a number of NexisClaw maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [NexisClaw/NexisClaw](https://github.com/NexisClaw/NexisClaw)
- **macOS desktop app** — [NexisClaw/NexisClaw](https://github.com/NexisClaw/NexisClaw) (apps/macos)
- **iOS app** — [NexisClaw/NexisClaw](https://github.com/NexisClaw/NexisClaw) (apps/ios)
- **Android app** — [NexisClaw/NexisClaw](https://github.com/NexisClaw/NexisClaw) (apps/android)
- **ClawHub** — [NexisClaw/clawhub](https://github.com/NexisClaw/clawhub)
- **Trust and threat model** — [NexisClaw/trust](https://github.com/NexisClaw/trust)

For issues that don't fit a specific repo, or if you're unsure, email **[security@NexisClaw.ai](mailto:security@NexisClaw.ai)** and we'll route it.

For NexisClaw core issues, submit through a private [GitHub Security Advisory](https://github.com/NexisClaw/NexisClaw/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
For full reporting instructions see our [Trust page](https://trust.NexisClaw.ai).
For maintainer response workflow, see the [incident response plan](docs/security/incident-response.md).

NexisClaw does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which NexisClaw trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an NexisClaw security boundary.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated NexisClaw impact.
...
- Tested version details (NexisClaw version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through NexisClaw. Showing that NexisClaw can reach a native parser is not enough by itself.
- Demonstrated impact tied to NexisClaw's documented trust boundaries.
- For exposed-secret reports: proof the credential is NexisClaw-owned (or grants access to NexisClaw-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example Sharp/libvips/libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through NexisClaw. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under NexisClaw's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
NexisClaw does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-NexisClaw-scopes` headers are ignored for that shared-secret path
...
- NexisClaw can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need NexisClaw, use one VPS (or host/OS user boundary) per user.
...
- This is expected in NexisClaw's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of NexisClaw's trusted computing base for a gateway.
...
- Using NexisClaw in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped NexisClaw production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.NexisClaw`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not NexisClaw-owned and not granting access to NexisClaw-operated infrastructure/services) without demonstrated NexisClaw impact
...
NexisClaw security guidance assumes:

- The host where NexisClaw runs is within a trusted OS/admin boundary.
- Anyone who can modify `~/.NexisClaw` state/config (including `NexisClaw.json`) is effectively a trusted operator.
...
NexisClaw's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
NexisClaw separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when NexisClaw can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the NexisClaw process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

NexisClaw uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/NexisClaw` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/NexisClaw` (or `NexisClaw-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the NexisClaw-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use NexisClaw temp helpers (`resolvePreferredNexisClawTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-NexisClaw-dir.ts`
...
For threat model + hardening guidance (including `NexisClaw security audit --deep` and `--fix`), see:
...
NexisClaw's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `NexisClaw gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - NexisClaw keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `NexisClaw security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__NexisClaw__/canvas/`, `/__NexisClaw__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__NexisClaw__/canvas/`, `/__NexisClaw__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

NexisClaw requires **Node.js 22.16.0 or later** (LTS). This version includes important security patches:
...
When running NexisClaw in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v NexisClaw-data:/app/data \
  NexisClaw/NexisClaw:latest
```

## Security Scanning

NexisClaw uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

NexisClaw runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
