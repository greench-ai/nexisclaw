...
export function listQaRunnerCliContributions(): readonly QaRunnerCliContribution[] {
  ...
  for (const plugin of listDeclaredQaRunnerPlugins(env)) {
    ...
    for (const runner of plugin.qaRunners) {
      ...
      if (!registration) {
        throw new Error(
          `QA runner plugin "${plugin.id}" declared "${runner.commandName}" in NexisClaw.plugin.json but did not export a matching CLI registration`,
        );
      }
      contributions.set(runner.commandName, {
        pluginId: plugin.id,
    ...
    for (const commandName of runtimeRegistrationByCommandName?.keys() ?? []) {
      if (!declaredCommandNames.has(commandName)) {
        throw new Error(
          `QA runner plugin "${plugin.id}" exported "${commandName}" from runtime-api.js but did not declare it in NexisClaw.plugin.json`,
        );
      }
    }
  }
