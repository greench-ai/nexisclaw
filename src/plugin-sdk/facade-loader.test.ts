...
const originalBundledPluginsDir = process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.NEXUSCLAW_DISABLE_BUNDLED_PLUGINS;
const FACADE_LOADER_GLOBAL = "__nexusclawTestLoadBundledPluginPublicSurfaceModuleSync";
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
  if (originalDisableBundledPlugins === undefined) {
    delete process.env.NEXUSCLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.NEXUSCLAW_DISABLE_BUNDLED_PLUGINS = originalDisableBundledPlugins;
  }
});

describe("plugin-sdk facade loader", () => {
  it("honors trusted bundled plugin dir overrides under the package root", () => {
    const pluginId = nextTrustedPluginId("nexusclaw-facade-loader-override-");
    const overrideA = createBundledPluginFixture({
      pluginId,
      kind: "dist",
      prefix: "nexusclaw-facade-loader-a-",
      marker: "override-a",
    });
    const overrideB = createBundledPluginFixture({
      pluginId,
      kind: "dist-runtime",
      prefix: "nexusclaw-facade-loader-b-",
      marker: "override-b",
    });

    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = overrideA.bundledPluginsDir;
    ...
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = overrideB.bundledPluginsDir;
  ...
  it("falls back to package source surfaces when an override dir lacks a bundled plugin", () => {
    const fixture = createPackageSourcePluginFixture({
      prefix: "nexusclaw-facade-loader-source-fallback-",
      marker: "source-fallback",
    });
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = createTempDirSync("nexusclaw-facade-loader-empty-");
  ...
  it("keeps bundled facade loads disabled when bundled plugins are disabled", () => {
    process.env.NEXUSCLAW_DISABLE_BUNDLED_PLUGINS = "1";
    delete process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR;
  ...
  it("shares loaded facade ids with facade-runtime", () => {
    const fixture = createBundledPluginFixture({
      prefix: "nexusclaw-facade-loader-ids-",
      marker: "identity-check",
    });
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
  ...
  it("uses native require for Windows dist facade loads", () => {
    const fixture = createBundledPluginFixture({
      prefix: "nexusclaw-facade-loader-windows-",
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const fixture = createCircularPluginFixture("nexusclaw-facade-loader-circular-");
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const fixture = createThrowingPluginFixture("nexusclaw-facade-loader-throw-");
    process.env.NEXUSCLAW_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
