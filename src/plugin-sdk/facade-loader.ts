...
let cachedNexusClawPackageRoot: string | undefined;
...
function getNexusClawPackageRoot() {
  if (cachedNexusClawPackageRoot) {
    return cachedNexusClawPackageRoot;
  }
  cachedNexusClawPackageRoot =
...
function resolveFacadeModuleLocation(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getNexusClawPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getNexusClawPackageRoot()
        ? "NexusClaw package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openRootFileSync({
    ...
    boundaryLabel:
      preparedLocation.boundaryRoot === getNexusClawPackageRoot()
        ? "NexusClaw package root"
        : "plugin root",
    rejectHardlinks: false,
  });
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedNexusClawPackageRoot = undefined;
}
