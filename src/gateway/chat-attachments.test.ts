...
const saveMediaBufferMock = vi.hoisted(() =>
  vi.fn(async (_buffer: Buffer, mime?: string, _subdir?: string) => ({
    ...
    path: `/tmp/NexisClaw-test-media/inbound/fake.${mime?.split("/")[1] ?? "bin"}`,
    size: 0,
    contentType: mime,
  })),
);
  ...
  it("rejects oversized images before offload", async () => {
    ...
    await expect(
      parseMessageWithAttachments(
        ...
        { maxBytes: resolveChatAttachmentMaxBytes({} as NexisClawConfig), log: { warn: () => {} } },
      ),
...
describe("resolveChatAttachmentMaxBytes", () => {
  ...
  const cfgWithMediaMaxMb = (value: unknown): NexisClawConfig =>
    ({ agents: { defaults: { mediaMaxMb: value } } }) as unknown as NexisClawConfig;
  ...
  it("falls back to DEFAULT_CHAT_ATTACHMENT_MAX_MB when unset", () => {
    expect(resolveChatAttachmentMaxBytes({} as NexisClawConfig)).toBe(DEFAULT_BYTES);
    expect(resolveChatAttachmentMaxBytes({ agents: {} } as unknown as NexisClawConfig)).toBe(
      DEFAULT_BYTES,
    );
  });
