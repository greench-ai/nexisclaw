...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NEXISCLAW_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.NEXISCLAW_DISABLE_BONJOUR) &&
  ...
  const tailnetDns = needsTailnetDns
    ...
    : undefined;
  const sshPortEnv = mdnsMinimal ? undefined : process.env.NEXISCLAW_SSH_PORT?.trim();
