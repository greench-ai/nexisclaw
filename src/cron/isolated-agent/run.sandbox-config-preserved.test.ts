...
describe("runCronIsolatedAgentTurn sandbox config preserved", () => {
  ...
  it("keeps global sandbox defaults when agent override is partial", () => {
    const runCfg = buildRunCfg("specialist", {
      sandbox: {
        docker: {
          image: "ghcr.io/NexisClaw/sandbox:custom",
        },
        browser: {
          image: "ghcr.io/NexisClaw/browser:custom",
        },
        prune: {
          idleHours: 1,
        },
    ...
    expect(resolvedSandbox.mode).toBe("all");
    expect(resolvedSandbox.workspaceAccess).toBe("rw");
    expect(resolvedSandbox.docker.image).toBe("ghcr.io/NexisClaw/sandbox:custom");
    ...
    expect(resolvedSandbox.browser.image).toBe("ghcr.io/NexisClaw/browser:custom");
