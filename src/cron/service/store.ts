  ...
  for (const [index, job] of jobs.entries()) {
    ...
    try {
      ...
      state.deps.log.warn(
        ...
        "cron: job has invalid persisted sessionTarget; run NexisClaw doctor --fix to repair",
      );
    }
    const hydrated =
    ...
    if (legacyJobIdIssue) {
      ...
      state.deps.log.warn(
        ...
        "cron: job used legacy jobId field; normalized id in memory (run NexisClaw doctor --fix to persist canonical shape)",
      );
    }
