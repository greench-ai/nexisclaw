...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("NexisClaw-camera-snap-");
    expect(mediaPaths[1]).toContain("NexisClaw-camera-snap-");
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /NexisClaw-camera-clip-front-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /NexisClaw-camera-snap-front-.*\.jpg$/,
      },
      {
