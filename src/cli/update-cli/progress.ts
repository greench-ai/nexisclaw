...
const STEP_LABELS: Record<string, string> = {
  ...
  "NexisClaw doctor entry": "Checking doctor entrypoint",
  "NexisClaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g NexisClaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g NexisClaw@latest --omit=optional");
  }
