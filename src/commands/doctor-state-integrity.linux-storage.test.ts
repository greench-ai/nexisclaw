...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.NexisClaw", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.NexisClaw",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.NexisClaw", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/NexisClaw-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/NexisClaw/.NexisClaw",
    });

    expect(result).toEqual({
      path: "/mnt/slow/NexisClaw/.NexisClaw",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".NexisClaw"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.NexisClaw`;
