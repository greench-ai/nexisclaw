...
export const resolveNexisClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadNexisClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/NexisClaw.json",
    createConfigIO,
...
vi.mock("NexisClaw/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("NexisClaw/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveNexisClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "NexisClaw-doctor-state-"));
