...
const SETUP_RE =
  /^(?:setup(?!\s+agent\b)|set\s+me\s+up|set\s+up\s+NexisClaw|onboard|onboard\s+me|bootstrap|first\s+run)(?:\b|$)/i;
...
function formatSetupPlanDescription(
  ...
  return `bootstrap NexisClaw setup for workspace ${workspace}${model}`;
}
