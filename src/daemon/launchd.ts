...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by NexisClaw. Do not edit while the gateway service is installed.",
...
function throwBootstrapGuiSessionError(params: {
  ...
  throw new Error(
    [
      ...
      "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.NexisClaw.ai/gateway",
    ].join("\n"),
  );
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `NexisClaw gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "NexisClaw gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "NexisClaw gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `NexisClaw gateway restart` is an explicit operator request to bring the
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    serviceTarget,
    plistPath,
    actionHint: "NexisClaw gateway restart",
  });
  writeLaunchAgentActionLine(stdout, "Restarted LaunchAgent", serviceTarget);
  return { outcome: "completed" };
}
