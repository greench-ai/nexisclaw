...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers NEXISCLAW_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.NEXISCLAW_GATEWAY_TOKEN;
    process.env.NEXISCLAW_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps NEXISCLAW_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.NEXISCLAW_GATEWAY_TOKEN;
    process.env.NEXISCLAW_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.NEXISCLAW_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "NEXISCLAW_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
