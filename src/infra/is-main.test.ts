...
describe("isMainModule", () => {
  ...
  it("returns true for configured wrapper-to-entry pairs", () => {
    expect(
      isMainModule({
        ...
        argv: ["node", "/repo/NexisClaw.mjs"],
        cwd: "/repo",
        env: {},
        wrapperEntryPairs: [{ wrapperBasename: "NexisClaw.mjs", entryBasename: "entry.js" }],
      }),
    ).toBe(true);
  });
  ...
  it("returns false when this module is only imported under PM2", () => {
    expect(
      isMainModule({
        currentFile: "/repo/node_modules/NexisClaw/dist/index.js",
