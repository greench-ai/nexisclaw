...
vi.mock("./NexisClaw-root.js", () => ({
  resolveNexisClawPackageRoot: vi.fn(async () => null),
  resolveNexisClawPackageRootSync: vi.fn(() => null),
}));
...
let NexisClawRoot: typeof import("./NexisClaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    NexisClawRoot = await import("./NexisClaw-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/NexisClaw");
    const wrapperArgv1 = abs("fixtures/bin/NexisClaw");
  ...
  it("uses resolveNexisClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/NexisClaw");
    (
      NexisClawRoot.resolveNexisClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/NexisClaw"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "NexisClaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "NexisClaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-NexisClaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/NexisClaw-bundle");
    (
      NexisClawRoot.resolveNexisClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/NexisClaw.app/Contents/MacOS/NexisClaw");
    const bundledUiDir = abs("fixtures/NexisClaw.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/NexisClaw-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
