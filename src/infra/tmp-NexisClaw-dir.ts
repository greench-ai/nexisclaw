...
export const POSIX_NEXISCLAW_TMP_DIR = "/tmp/NexisClaw";
...
export type ResolvePreferredNexisClawTmpDirOptions = {
...
export function resolvePreferredNexisClawTmpDir(
  options: ResolvePreferredNexisClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "NexisClaw" : `NexisClaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[NexisClaw] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback NexisClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback NexisClaw temp dir: ${fallbackPath}`);
    }
