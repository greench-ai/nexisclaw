...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__NexisClaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "NexisClaw-root");
...
function setPackageRoot(root: string, name = "NexisClaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./NexisClaw-root.js").resolveNexisClawPackageRootSync,
  asyncResolver: typeof import("./NexisClaw-root.js").resolveNexisClawPackageRoot,
  opts: Parameters<typeof import("./NexisClaw-root.js").resolveNexisClawPackageRootSync>[0],
...
vi.mock("./NexisClaw-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveNexisClawPackageRoot", () => {
  let resolveNexisClawPackageRoot: typeof import("./NexisClaw-root.js").resolveNexisClawPackageRoot;
  let resolveNexisClawPackageRootSync: typeof import("./NexisClaw-root.js").resolveNexisClawPackageRootSync;
  let clearNexisClawPackageRootCaches: typeof import("./NexisClaw-root.js").__testing.clearNexisClawPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveNexisClawPackageRoot,
      resolveNexisClawPackageRootSync,
      __testing: { clearNexisClawPackageRootCaches },
    } = await import("./NexisClaw-root.js"));
  });

  beforeEach(() => {
    clearNexisClawPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "NexisClaw");
        const pkgRoot = path.join(project, "node_modules", "NexisClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "NexisClaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "NexisClaw.mjs")));
    ...
    {
      name: "falls through from a non-NexisClaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-NexisClaw");
    ...
    {
      name: "returns null for non-NexisClaw package roots",
      setup: () => {
        const pkgRoot = fx("not-NexisClaw");
        setPackageRoot(pkgRoot, "not-NexisClaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "NexisClaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "NexisClaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
