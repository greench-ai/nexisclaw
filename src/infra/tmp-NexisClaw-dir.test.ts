...
import { POSIX_NEXISCLAW_TMP_DIR, resolvePreferredNexisClawTmpDir } from "./tmp-NexisClaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredNexisClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `NexisClaw-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredNexisClawTmpDir({
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredNexisClawTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredNexisClawTmpDir", () => {
  it("prefers /tmp/NexisClaw when it already exists and is writable", () => {
  ...
  it("prefers /tmp/NexisClaw when it does not exist but /tmp is writable", () => {
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/NexisClaw when /tmp/NexisClaw is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/NexisClaw when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/NexisClaw exists but is not writable",
    ...
    {
      name: "falls back when /tmp/NexisClaw is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/NexisClaw is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/NexisClaw is group/other writable",
  ...
  it("repairs existing /tmp/NexisClaw permissions when they are too broad", () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      `[NexisClaw] tightened permissions on temp dir: ${POSIX_NEXISCLAW_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/NexisClaw after create when the initial mode stays too broad", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback NexisClaw temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "NexisClaw");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[NexisClaw] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/NexisClaw when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/NexisClaw when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_NEXISCLAW_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback NexisClaw temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/NexisClaw` and routed
    // log files / TTS temp files there instead of `%TEMP%\NexisClaw`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "NexisClaw-501");
    ...
    const result = resolvePreferredNexisClawTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
