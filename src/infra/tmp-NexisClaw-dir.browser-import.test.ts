...
describe("tmp-NexisClaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__NexisClawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_NEXISCLAW_TMP_DIR, resolvePreferredNexisClawTmpDir } from "./src/infra/tmp-NexisClaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_NEXISCLAW_TMP_DIR,
            resolverType: typeof resolvePreferredNexisClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-NexisClaw-dir-browser-entry.ts",
      },
      write: false,
    });
