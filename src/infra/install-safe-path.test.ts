...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@NexisClaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@NexisClaw/matrix", id: "matrix", expected: true },
    { packageName: "@NexisClaw/matrix", id: "@NexisClaw/matrix", expected: true },
    { packageName: "@NexisClaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@NexisClaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@NexisClaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "NexisClaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "NexisClaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "NexisClaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "NexisClaw-install-safe-" }, async (parentDir) => {
