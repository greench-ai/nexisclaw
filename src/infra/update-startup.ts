...
import { resolveNexisClawPackageRoot } from "./NexisClaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: NexisClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: NexisClawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("NexisClaw update")}`,
      );
