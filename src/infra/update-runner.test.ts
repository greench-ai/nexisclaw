...
const fixtureRootTracker = createSuiteTempRootTracker({ prefix: "NexisClaw-update-" });
...
describe("runGatewayUpdate", () => {
  const preflightPrefixPattern = /(?:NexisClaw-update-preflight-|ocu-pf-)/;
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(path.join(tempDir, "NexisClaw.mjs"), "export {};\n", "utf-8");
  });
  ...
  async function createStableTagRunner(params: {
    ...
    const doctorKey = `${doctorNodePath} ${path.join(tempDir, "NexisClaw.mjs")} doctor --non-interactive --fix`;
  ...
  async function setupGitCheckout(options?: { packageManager?: string }) {
    ...
    const pkg: Record<string, string> = { name: "NexisClaw", version: "1.0.0" };
  ...
  function createGlobalNpmUpdateRunner(params: {
    ...
    const baseInstallKey = "npm i -g NexisClaw@latest --no-fund --no-audit --loglevel=error";
    const omitOptionalInstallKey =
      "npm i -g NexisClaw@latest --omit=optional --no-fund --no-audit --loglevel=error";
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${doctorNodePath} ${path.join(tempDir, "NexisClaw.mjs")} doctor --non-interactive --fix`]: {
        stdout: "",
      },
    });
  ...
  it("bootstraps pnpm via npm when pnpm and corepack are unavailable", async () => {
    ...
    const { calls, runCommand } = createGitInstallRunner({
      ...
      doctorCommand: `${process.execPath} ${path.join(tempDir, "NexisClaw.mjs")} doctor --non-interactive`,
      onCommand: (key, options) => {
        if (key === "pnpm --version") {
          const envPath = options?.env?.PATH ?? options?.env?.Path ?? "";
          if (envPath.includes("NexisClaw-update-pnpm-")) {
  ...
  it("uses npm-bootstrapped pnpm for dev preflight when pnpm and corepack are missing", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(tempDir, "NexisClaw.mjs")} doctor --non-interactive --fix`;
    ...
    expect(pnpmEnvPaths.some((envPath) => envPath.includes("NexisClaw-update-pnpm-"))).toBe(true);
  });
  ...
  it("retries windows pnpm git installs with --ignore-scripts for dev updates", async () => {
    ...
    try {
      const runCommand = async (
        ...
        if (key === "pnpm install") {
          if (options?.cwd && /(?:NexisClaw-update-preflight-|ocu-pf-)/.test(options.cwd)) {
  ...
  it("falls back to the cloned cwd when git root probing misses a fresh checkout", async () => {
    ...
    const doctorNodePath = await resolveStableNodePath(process.execPath);
    const doctorCommand = `${doctorNodePath} ${path.join(gitRoot, "NexisClaw.mjs")} doctor --non-interactive --fix`;
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      ...
      JSON.stringify({ name: "NexisClaw", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  async function runNpmGlobalUpdateCase(params: {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async () => {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "NexisClaw", version: "2.0.0" }),
          "utf-8",
        );
      },
    });
        ...
        if (normalizedInstallCommand === params.installCommand) {
          const packageRoot =
            process.platform === "win32"
              ? path.join(installPrefix, "node_modules", "NexisClaw")
              : path.join(installPrefix, "lib", "node_modules", "NexisClaw");
  ...
  it.each([
    {
      ...
      expectedInstallCommand: "npm i -g NexisClaw@latest --no-fund --no-audit --loglevel=error",
    },
    {
      title: "uses update channel for global npm installs when tag is omitted",
      expectedInstallCommand: "npm i -g NexisClaw@beta --no-fund --no-audit --loglevel=error",
  ...
  it("updates global npm installs from the GitHub main package spec", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand:
        "npm i -g github:NexisClaw/NexisClaw#main --no-fund --no-audit --loglevel=error",
      tag: "main",
    });
  ...
  it("runs doctor after global npm updates before reporting success", async () => {
    ...
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: "npm i -g NexisClaw@latest --no-fund --no-audit --loglevel=error",
      onInstall: async () => {
    ...
    expect(result.steps.map((step) => step.name)).toContain("NexisClaw doctor");
  ...
  it("fails global npm updates when post-update doctor fails", async () => {
    ...
    expect(lastStep?.name).toBe("NexisClaw doctor");
  ...
  it("falls back to global npm update when git is missing from PATH", async () => {
    ...
    expect(result.status).toBe("ok");
    expect(result.mode).toBe("npm");
    expect(calls).toContain("npm i -g NexisClaw@latest --no-fund --no-audit --loglevel=error");
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "NexisClaw");
    const staleDir = path.join(nodeModules, ".NexisClaw-stale");
  ...
  it("fails global npm update when the installed version misses the requested correction", async () => {
    const { calls, result } = await runNpmGlobalUpdateCase({
      expectedInstallCommand: "npm i -g NexisClaw@2026.3.23-2 --no-fund --no-audit --loglevel=error",
      tag: "2026.3.23-2",
    });
    ...
    expect(calls).toContain("npm i -g NexisClaw@2026.3.23-2 --no-fund --no-audit --loglevel=error");
  });

  it("fails global npm update when bundled runtime sidecars are missing after install", async () => {
    const { nodeModules, pkgRoot } = await createGlobalPackageFixture(tempDir);
    const expectedInstallCommand = "npm i -g NexisClaw@latest --no-fund --no-audit --loglevel=error";
    ...
    const { runCommand } = createGlobalInstallHarness({
      ...
      onInstall: async (options) => {
        ...
        if (options?.installPrefix) {
          ...
          await fs.writeFile(path.join(binDir, "NexisClaw"), "#!/bin/sh\n", "utf-8");
        }
      },
    });
  ...
  it("uses NEXISCLAW_UPDATE_PACKAGE_SPEC for global package updates", async () => {
    ...
    const expectedInstallCommand =
      "npm i -g http://10.211.55.2:8138/NexisClaw-next.tgz --no-fund --no-audit --loglevel=error";
    const { calls, runCommand } = createGlobalInstallHarness({
      pkgRoot,
      npmRootOutput: nodeModules,
      installCommand: expectedInstallCommand,
  ...
  it("updates global bun installs when detected", async () => {
    ...
    await withEnvAsync({ BUN_INSTALL: bunInstall }, async () => {
      ...
      const { calls, runCommand } = createGlobalInstallHarness({
        pkgRoot,
        installCommand: "bun add -g NexisClaw@latest",
        onInstall: async () => {
          await writeGlobalPackageVersion(pkgRoot);
        },
      });
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls).toContain("bun add -g NexisClaw@latest");
    });
  });

  it("rejects git roots that are not a NexisClaw checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-NexisClaw-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when NexisClaw.mjs is missing", async () => {
    await setupGitCheckout({ packageManager: "pnpm@8.0.0" });
    await fs.rm(path.join(tempDir, "NexisClaw.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("NexisClaw doctor entry");
  });
