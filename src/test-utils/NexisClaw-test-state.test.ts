...
import { createNexisClawTestState, withNexisClawTestState } from "./NexisClaw-test-state.js";
...
describe("NexisClaw test state", () => {
  it("creates an isolated home layout with spawn env and restores process env", async () => {
    ...
    const state = await createNexisClawTestState({
      label: "unit",
      scenario: "minimal",
    });
    ...
    try {
      ...
      expect(state.stateDir).toBe(path.join(state.home, ".NexisClaw"));
      expect(state.configPath).toBe(path.join(state.stateDir, "NexisClaw.json"));
    ...
    expect(process.env.NEXISCLAW_HOME).toBe(previousNexisClawHome);
  ...
  it("clears inherited agent-dir overrides by default", async () => {
    ...
    process.env.NEXISCLAW_AGENT_DIR = "/tmp/outside-NexisClaw-agent";
    ...
    try {
      ...
      expect(process.env.NEXISCLAW_AGENT_DIR).toBe("/tmp/outside-NexisClaw-agent");
  ...
  it("allows explicit agent-dir overrides when a test needs them", async () => {
    await withNexisClawTestState(
      {
        env: {
          NEXISCLAW_AGENT_DIR: "/tmp/explicit-NexisClaw-agent",
          PI_CODING_AGENT_DIR: "/tmp/explicit-pi-agent",
        },
      },
      async (state) => {
        expect(process.env.NEXISCLAW_AGENT_DIR).toBe("/tmp/explicit-NexisClaw-agent");
        expect(process.env.PI_CODING_AGENT_DIR).toBe("/tmp/explicit-pi-agent");
        expect(state.env.NEXISCLAW_AGENT_DIR).toBe("/tmp/explicit-NexisClaw-agent");
