...
type NexisClawTestStateLayout = "home" | "state-only" | "split";

type NexisClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type NexisClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: NexisClawTestStateLayout;
  scenario?: NexisClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type NexisClawTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "NexisClaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "NexisClaw.json"),
...
function scenarioConfig(options: NexisClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "NexisClaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: NexisClawTestStateOptions): Record<string, string | undefined> {
...
export async function createNexisClawTestState(
  options: NexisClawTestStateOptions = {},
): Promise<NexisClawTestState> {
  ...
  const state: NexisClawTestState = {
    root,
    ...paths,
    env,
    envVars,
...
export async function withNexisClawTestState<T>(
  options: NexisClawTestStateOptions,
  fn: (state: NexisClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createNexisClawTestState(options);
  try {
