...
const hoisted = await vi.hoisted(async () => {
  ...
  return {
    ...
    exportTrajectoryBundleMock: vi.fn(() => ({
      outputDir: "/tmp/workspace/.NexisClaw/trajectory-exports/NexisClaw-trajectory-session",
      manifest: {
        eventCount: 7,
        runtimeEventCount: 3,
    ...
    resolveDefaultTrajectoryExportDirMock: vi.fn(
      () => "/tmp/workspace/.NexisClaw/trajectory-exports/NexisClaw-trajectory-session",
    ),
    accessMock: vi.fn(
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "NexisClaw-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "NexisClaw sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryReply", () => {
  ...
  it("builds a trajectory bundle from the target session", async () => {
    ...
    expect(String(exportParams.workspaceDir)).toContain("NexisClaw-export-command-");
  });
  ...
  it("keeps user-named output paths inside the workspace trajectory export directory", async () => {
    ...
    expect(exportBundleParams().outputDir).toBe(
      path.join(params.workspaceDir, ".NexisClaw", "trajectory-exports", "my-bundle"),
    );
  });
  ...
  it("rejects output paths redirected by a symlinked exports directory", async () => {
    ...
    fs.mkdirSync(path.join(workspaceDir, ".NexisClaw"), { recursive: true });
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".NexisClaw", "trajectory-exports"));
  ...
  it("rejects symlinked state directories before creating export folders", async () => {
    const workspaceDir = makeTempDir();
    const outsideDir = makeTempDir();
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".NexisClaw"));
    const params = makeParams(workspaceDir);
    params.command.commandBodyNormalized = "/export-trajectory my-bundle";
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.NexisClaw.ai/tools/trajectory");
    ...
    expect(command).not.toContain("NexisClaw sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("NexisClaw-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("NexisClaw sessions export-trajectory");
