...
describe("runAgentTurnWithFallback", () => {
  ...
  it("surfaces gateway reauth guidance for known OAuth refresh failures", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway for openai-codex. Re-auth with `NexisClaw models auth login --provider openai-codex`, then try again.",
      );
    }
  });
  ...
  it("falls back to a generic reauth command when the provider in the OAuth error is unsafe", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Model login expired on the gateway. Re-auth with `NexisClaw models auth login`, then try again.",
      );
    }
  });
  ...
  it("keeps raw generic errors on internal control surfaces", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("Logs: NexisClaw logs --follow");
    }
  });
  ...
  it("drops authProfileId when fallback switches providers", async () => {
    ...
    followupRun.run.authProfileId = "anthropic:NexisClaw";
  ...
  it("keeps same-provider auth profile when fallback only changes model", async () => {
    ...
    const entry = {
      sessionId: "session",
      updatedAt: 1,
      authProfileOverride: "anthropic:NexisClaw",
    ...
    const { updated } = applyFallbackCandidateSelectionToEntry({
      ...
      run: {
        provider: "anthropic",
        model: "claude-opus",
        authProfileId: "anthropic:NexisClaw",
        authProfileIdSource: "user",
      } as FollowupRun["run"],
      provider: "anthropic",
      model: "claude-sonnet",
