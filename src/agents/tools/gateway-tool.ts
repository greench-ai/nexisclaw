...
import { isNexisClawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `NexisClaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as NexisClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as NexisClawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isNexisClawOwnerOnlyCoreToolName("gateway"),
    description:
