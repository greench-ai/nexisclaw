...
export function createCronTool(opts?: CronToolOptions, deps?: CronToolDeps): AnyAgentTool {
  ...
  return {
    label: "Cron",
    name: "cron",
    ownerOnly: isNexisClawOwnerOnlyCoreToolName("cron"),
...
Main-session cron jobs enqueue system events for heartbeat handling. Isolated cron jobs create background task runs that appear in \`NexisClaw tasks\`.
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "add": {
          ...
          // See: https://github.com/NexisClaw/NexisClaw/issues/11310
