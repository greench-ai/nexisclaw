...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-test-"));
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: NexisClaw --profile isolated doctor --non-interactive",
          );
        },
      );
    } finally {
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/NexisClaw-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/NexisClaw-bin"] } } }',
      }),
    ).rejects.toThrow(
