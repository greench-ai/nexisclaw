...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "NEXISCLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { NEXISCLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "NEXISCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { NEXISCLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "NEXISCLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
