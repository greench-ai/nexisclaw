...
import * as openClawPluginTools from "./NexisClaw-plugin-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createNexisClawCodingTools>) {
...
type NexisClawCodingTool = ReturnType<typeof createNexisClawCodingTools>[number];
type NexisClawToolsOptions = NonNullable<Parameters<typeof createNexisClawTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: NexisClawCodingTool[], name: string): NexisClawCodingTool {
...
function requireToolExecute(tool: NexisClawCodingTool): NonNullable<NexisClawCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateNexisClawToolsOptions(): NexisClawToolsOptions {
  const calls = vi.mocked(createNexisClawTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createNexisClawTools call");
  }
  return options;
}
...
describe("createNexisClawCodingTools", () => {
  const testConfig: NexisClawConfig = {};

  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createNexisClawCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("keeps PI Tool Search controls when core NexisClaw tools are not materialized", () => {
    const createNexisClawToolsMock = vi.mocked(createNexisClawTools);
    createNexisClawToolsMock.mockClear();
    ...
    expect(createNexisClawToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createNexisClawToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateNexisClawToolsOptions();
    expectListIncludes(options.pluginToolAllowlist, ["memory_search", "memory_get"]);
  });

  it("passes source reply delivery mode to NexisClaw tool construction", () => {
    const createNexisClawToolsMock = vi.mocked(createNexisClawTools);
    createNexisClawToolsMock.mockClear();
    ...
    expect(createNexisClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNexisClawToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into NexisClaw tool construction plans", () => {
    const createNexisClawToolsMock = vi.mocked(createNexisClawTools);
    createNexisClawToolsMock.mockClear();
    ...
    expect(createNexisClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNexisClawToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the NexisClaw core factory", () => {
    const createNexisClawToolsMock = vi.mocked(createNexisClawTools);
    createNexisClawToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(openClawPluginTools, "resolveNexisClawPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createNexisClawToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNexisClawToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to NexisClaw tool factory planning", () => {
    const createNexisClawToolsMock = vi.mocked(createNexisClawTools);
    createNexisClawToolsMock.mockClear();
    ...
    expect(createNexisClawToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNexisClawToolsOptions().pluginToolDenylist, ["pdf"]);
  });
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("NexisClaw-tools:test-helper")).toBeLessThan(
      stages.indexOf("NexisClaw-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createNexisClawCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createNexisClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createNexisClawCodingTools({
      config: testConfig,
    ...
    const disabledConfig: NexisClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createNexisClawCodingTools({
    ...
    const anthropicTools = createNexisClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: NexisClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNexisClawCodingTools({
    ...
    const denied = createNexisClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createNexisClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createNexisClawCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNexisClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createNexisClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createNexisClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createNexisClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createNexisClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createNexisClawCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createNexisClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createNexisClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createNexisClawCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createNexisClawCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-read-"));
    try {
      ...
      const contents = "Hello from NexisClaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "NexisClaw-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "NexisClaw-workspace"),
    ...
    const tools = createNexisClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-canonical-"));
    try {
      const tools = createNexisClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-legacy-alias-"));
    try {
      const tools = createNexisClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-structured-write-"));
    try {
      const tools = createNexisClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-structured-edit-"));
    try {
