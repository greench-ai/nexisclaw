 ...
 * plugin skills directory (~/.NexisClaw/plugin-skills/) so the agent SDK can
 * discover them at the conventional file-system path.
 *
 * The plugin-skills directory is fully owned by NexisClaw — every entry is
...
function publishPluginSkills(skillDirs: string[], opts?: { pluginSkillsDir?: string }): void {
  ...
  // Plugin skill symlinks are owned by NexisClaw and publish at extra-dir
  ...
  // The plugin-skills directory is fully owned by NexisClaw: every entry is a
