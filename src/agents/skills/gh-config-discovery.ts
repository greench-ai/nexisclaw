...
// NexisClaw process is running with a different HOME (e.g. the per-agent
...
// See https://github.com/NexisClaw/NexisClaw/issues/78063.
...
export type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the NexisClaw service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the NexisClaw service environment, then restart the gateway.`,
  );
  return lines;
}
