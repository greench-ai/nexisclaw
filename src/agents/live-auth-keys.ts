...
const GOOGLE_LIVE_SINGLE_KEY = "NEXISCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NEXISCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "NEXISCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NEXISCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NEXISCLAW_LIVE_${base}_KEY`;
