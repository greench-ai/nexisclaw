...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  it("merges user-configured mcp.servers from NexisClaw config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "NexisClaw-cli-bundle-mcp-user-servers-",
    );
  ...
  it("translates NexisClaw transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "NexisClaw-cli-bundle-mcp-user-servers-transport-",
    );
  ...
  it("preserves explicit type and still strips transport on user mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "NexisClaw-cli-bundle-mcp-user-servers-transport-explicit-",
    );
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "NexisClaw-cli-bundle-mcp-user-servers-loopback-",
    );
  ...
  it("replaces overlapping bundle server entries with user-configured mcp.servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "NexisClaw-cli-bundle-mcp-user-servers-replace-",
    );
