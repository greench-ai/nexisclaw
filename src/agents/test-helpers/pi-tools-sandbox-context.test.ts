...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("NexisClaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("NexisClaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("NexisClaw-sbx-");
