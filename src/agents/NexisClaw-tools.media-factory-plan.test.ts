...
import { resolveOptionalMediaToolFactoryPlan } from "./NexisClaw-tools.media-factory-plan.js";
...
type CreateNexisClawToolsOptions = Parameters<
  typeof import("./NexisClaw-tools.js").createNexisClawTools
>[0];

async function createNexisClawToolsForTest(options?: CreateNexisClawToolsOptions) {
  const { createNexisClawTools } = await import("./NexisClaw-tools.js");
  return createNexisClawTools(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/NexisClaw.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): NexisClawConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  ...
  it("preserves implicit allow-all from alsoAllow-only policies for built-in media factories", async () => {
    ...
    const toolNames = (
      await createNexisClawToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: NexisClawConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createNexisClawToolsForTest({
      config,
      agentDir: "/tmp/NexisClaw-agent-main",
