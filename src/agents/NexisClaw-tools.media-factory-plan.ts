...
function hasExplicitImageModelConfig(config: NexisClawConfig | undefined): boolean {
  return hasExplicitToolModelConfig(config?.agents?.defaults?.imageModel);
}

function hasExplicitPdfModelConfig(config: NexisClawConfig | undefined): boolean {
  return (
