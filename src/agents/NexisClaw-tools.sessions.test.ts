...
import "./test-helpers/fast-NexisClaw-tools-sessions.js";
...
function createNexisClawTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createNexisClawTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "NexisClaw-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createNexisClawTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
