...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "NexisClaw-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "NexisClaw-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredNexisClawTmpDir() ?? os.tmpdir());
}
