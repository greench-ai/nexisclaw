...
} from "./NexisClaw-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./NexisClaw-tools.nodes-workspace-guard.js";
import {
  collectPresentNexisClawTools,
  isUpdatePlanToolEnabledForNexisClawTools,
} from "./NexisClaw-tools.registration.js";
import {
  type HookContext,
...
type NexisClawToolsDeps = {
  callGateway: typeof callGateway;
  config?: NexisClawConfig;
};

const defaultNexisClawToolsDeps: NexisClawToolsDeps = {
  callGateway,
};

let openClawToolsDeps: NexisClawToolsDeps = defaultNexisClawToolsDeps;

export function createNexisClawTools(
  options?: {
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("NexisClaw-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("NexisClaw-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("NexisClaw-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("NexisClaw-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("NexisClaw-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:nodes-tool");
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentNexisClawTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentNexisClawTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...collectPresentNexisClawTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("NexisClaw-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("NexisClaw-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("NexisClaw-tools:tool-hooks");
...
export const __testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<NexisClawToolsDeps>) {
    openClawToolsDeps = overrides
      ? {
          ...defaultNexisClawToolsDeps,
          ...overrides,
        }
      : defaultNexisClawToolsDeps;
  },
