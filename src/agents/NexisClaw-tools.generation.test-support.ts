...
export function describeNexisClawGenerationToolRegistration(params: {
  suiteName: string;
  toolName: string;
  toolLabel: string;
}) {
  describe(params.suiteName, () => {
    it(`registers ${params.toolName} when ${params.toolLabel} is present`, () => {
      ...
      expect(collectPresentNexisClawTools([tool])).toEqual([tool]);
    });

    it(`omits ${params.toolName} when ${params.toolLabel} is absent`, () => {
      expect(collectPresentNexisClawTools([null]).map((tool) => tool.name)).not.toContain(
        params.toolName,
      );
    });
  });
