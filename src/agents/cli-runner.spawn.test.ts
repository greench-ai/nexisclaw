...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the NexisClaw skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("NexisClaw-cli-system-prompt-");
  ...
  it("passes NexisClaw skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("NexisClaw-skills");
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("NexisClaw-session");
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "NexisClaw-live-skills-"));
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "NexisClaw-cli-bootstrap-context-"),
    );
