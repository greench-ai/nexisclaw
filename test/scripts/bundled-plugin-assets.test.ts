...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "NexisClaw-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "NexisClaw.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@NexisClaw/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@NexisClaw/canvas-plugin",
          phase: "build",
