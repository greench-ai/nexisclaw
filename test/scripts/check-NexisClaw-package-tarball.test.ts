...
const CHECK_SCRIPT = "scripts/check-NexisClaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "NexisClaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "NexisClaw", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><NexisClaw-app></NexisClaw-app>",
    ...
    const tarball = join(root, "NexisClaw.tgz");
...
describe("check-NexisClaw-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("NexisClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
