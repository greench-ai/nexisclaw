...
} from "../scripts/NexisClaw-npm-release-check.ts";
import {
...
describe("parseNpmPackJsonOutput", () => {
  it("parses a plain npm pack JSON array", () => {
    expect(parseNpmPackJsonOutput('[{"filename":"NexisClaw.tgz","files":[]}]')).toEqual([
      { filename: "NexisClaw.tgz", files: [] },
    ]);
  });
  ...
  it("parses the trailing JSON payload after npm lifecycle logs", () => {
    const stdout = [
      ...
      "",
      "> NexisClaw@2026.3.23 prepack",
      "> pnpm build && pnpm ui:build",
      "",
      "[copy-hook-metadata] Copied 4 hook metadata files.",
      '[{"filename":"NexisClaw.tgz","files":[{"path":"dist/control-ui/index.html"}]}]',
    ].join("\n");

    expect(parseNpmPackJsonOutput(stdout)).toEqual([
      {
        filename: "NexisClaw.tgz",
  ...
  it("returns null when no JSON payload is present", () => {
    expect(parseNpmPackJsonOutput("> NexisClaw@2026.3.23 prepack")).toBeNull();
  });
});
...
describe("collectForbiddenPackedPathErrors", () => {
  ...
  it("rejects root dist chunks that still reference the private qa lab", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "NexisClaw-pack-private-qa-"));
  ...
  it("rejects private QA paths in the generated dist inventory", () => {
    const rootDir = mkdtempSync(join(tmpdir(), "NexisClaw-pack-inventory-"));
  ...
  it("validates the expected npm package metadata", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        license: "MIT",
        repository: { url: "git+https://github.com/NexisClaw/NexisClaw.git" },
        bin: { NexisClaw: "NexisClaw.mjs" },
      }),
    ).toStrictEqual([]);
  });
  ...
  it("rejects local fs-safe dependency specs for npm release", () => {
    expect(
      collectReleasePackageMetadataErrors({
        ...
        repository: { url: "git+https://github.com/NexisClaw/NexisClaw.git" },
        bin: { NexisClaw: "NexisClaw.mjs" },
        dependencies: { "@NexisClaw/fs-safe": "link:../fs-safe" },
      }),
    ).toContain(
      'package.json dependencies["@NexisClaw/fs-safe"] must use a published semver range before npm release; found "link:../fs-safe".',
    );
  });
