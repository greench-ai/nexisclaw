...
function resolveRequiredHomeDir(
  ...
  const explicitHome = normalizeHomeValue(env.NEXUSCLAW_HOME);
...
export function resolveStateDir(
  ...
  if (env.NEXUSCLAW_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
...
function listAgentEntries(cfg: NexusClawConfig): AgentConfig[] {
  return Array.isArray(cfg.agents?.list)
    ? cfg.agents.list.filter((entry): entry is AgentConfig => Boolean(entry))
    : [];
}

function resolveDefaultAgentId(cfg: NexusClawConfig): string {
...
function resolveAgentConfig(cfg: NexusClawConfig, agentId: string): AgentConfig | undefined {
