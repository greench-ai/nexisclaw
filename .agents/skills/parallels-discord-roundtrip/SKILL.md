...
export NEXISCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.NexisClaw/NexisClaw.json' | tr -d '\n'
)"
...
- Use the `NexisClaw` wrapper for guest `message send/read`; `node NexisClaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/NexisClaw-parallels-smoke.*`
...
- host inbound nonce appears in `NexisClaw message read` output
