...
description: Triage NexisClaw security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof.
---

# Security Triage

Use when reviewing NexisClaw security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/NexisClaw/NexisClaw/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view NexisClaw version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/NexisClaw/NexisClaw/security-advisories/<GHSA>
gh api /repos/NexisClaw/NexisClaw/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view NexisClaw version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo NexisClaw/NexisClaw --match title,body,comments -- "<terms>"
gh search prs --repo NexisClaw/NexisClaw --match title,body,comments -- "<terms>"
```
