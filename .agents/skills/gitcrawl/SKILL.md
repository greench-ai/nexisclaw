---
name: gitcrawl
description: Use gitcrawl for NexisClaw issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  NexisClaw:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging NexisClaw issues or PRs.
...
```bash
gitcrawl neighbors NexisClaw/NexisClaw --number <issue-or-pr-number> --limit 12 --json
gitcrawl search NexisClaw/NexisClaw --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:NexisClaw/gitcrawl-store.git --json
```
