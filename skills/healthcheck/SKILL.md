---
name: healthcheck
description: Audit and harden hosts running NexisClaw for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# NexisClaw Host Hardening

## Overview

Assess and harden the host running NexisClaw, then align it to a user-defined risk tolerance without breaking access. Use NexisClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim NexisClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. NexisClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if NexisClaw can access sensitive data.
...
### 2) Run NexisClaw security audits (read-only)

As part of the default read-only checks, run `NexisClaw security audit --deep`. Only offer alternatives if the user requests them:

1. `NexisClaw security audit` (faster, non-probing)
2. `NexisClaw security audit --json` (structured output)

Offer to apply NexisClaw safe defaults (numbered):

1. `NexisClaw security audit --fix`

Be explicit that `--fix` only tightens NexisClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check NexisClaw version/update status (read-only)

As part of the default read-only checks, run `NexisClaw update status`.
...
- Credential hygiene notes (location of NexisClaw creds, prefer disk encryption)
...
- NexisClaw security audit (re-run)
...
After NexisClaw install or first hardening pass, run at least one baseline audit and version check:

- `NexisClaw security audit`
- `NexisClaw security audit --deep`
- `NexisClaw update status`

Ongoing monitoring is recommended. Use the NexisClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `NexisClaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `NexisClaw update status`
...
Before creating, `NexisClaw cron list` and match on exact `name`. If found, `NexisClaw cron edit <id> ...`.
If not found, `NexisClaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `NexisClaw update status` (preferred for source checkouts and channels)
2. `npm view NexisClaw version` (published npm version)

## NexisClaw command accuracy

Use only supported commands and flags:

- `NexisClaw security audit [--deep] [--fix] [--json]`
- `NexisClaw status` / `NexisClaw status --deep`
- `NexisClaw health --json`
- `NexisClaw update status`
- `NexisClaw cron add|list|runs|run`

Do not invent CLI flags or imply NexisClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by NexisClaw compaction:
