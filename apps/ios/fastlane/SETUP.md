...
ASC_KEYCHAIN_SERVICE=NexisClaw-asc-key
...
```bash
ASC_APP_IDENTIFIER=ai.NexisClaw.client
# or
...
- confirm `apps/ios/build/beta/NexisClaw-<version>.ipa` exists
...
- The beta flow regenerates `apps/ios/NexisClaw.xcodeproj` from `apps/ios/project.yml` before archiving
