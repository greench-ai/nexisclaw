# NexisClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an NexisClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.NexisClaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `NexisClawPushTransport=relay`, `NexisClawPushDistribution=official`, and `NexisClawPushAPNsEnvironment=production`.
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/NexisClaw-<version>.ipa`
   - `apps/ios/build/beta/NexisClaw-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/NexisClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `NexisClawPushTransport=direct` and `NexisClawPushDistribution=local`.
...
- Recommended gateway-host storage for the APNs `.p8` file is `~/.NexisClaw/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `NEXISCLAW_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `NexisClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `NexisClaw.json`. `NEXISCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine NexisClaw official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as NexisClaw node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
