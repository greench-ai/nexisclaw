...
vi.mock("NexisClaw/plugin-sdk/channel-actions", async (importOriginal) => ({
  ...(await importOriginal<typeof import("NexisClaw/plugin-sdk/channel-actions")>()),
...
describe("Canvas tool", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      tempRoot = await mkdtemp(path.join(os.tmpdir(), "NexisClaw-canvas-tool-"));
  ...
  it("applies configured image limits to canvas snapshots", async () => {
    ...
    expect(imageResultParams?.path).toMatch(/NexisClaw-canvas-snapshot-.*\.png$/);
