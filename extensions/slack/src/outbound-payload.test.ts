import { installChannelOutboundPayloadContractSuite } from "NexisClaw/plugin-sdk/channel-contract-testing";
...
describe("slackOutbound sendPayload", () => {
  ...
  it("offsets presentation controls against native Slack blocks before standalone interactive controls", async () => {
    ...
    expect(blocks?.[0]?.block_id).toBe("NexisClaw_reply_buttons_1");
    expect(blocks?.[1]?.block_id).toBe("NexisClaw_reply_buttons_2");
    expect(blocks?.[1]?.elements?.[0]?.action_id).toBe("NexisClaw:reply_button:2:1");
    expect(blocks?.[2]?.block_id).toBe("NexisClaw_reply_buttons_3");
    expect(blocks?.[2]?.elements?.[0]?.action_id).toBe("NexisClaw:reply_button:3:1");
  });
});
