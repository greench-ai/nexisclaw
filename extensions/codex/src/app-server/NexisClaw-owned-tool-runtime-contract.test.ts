...
import { wrapToolWithBeforeToolCallHook } from "NexisClaw/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installNexisClawOwnedToolHooks,
  mediaToolResult,
  resetNexisClawOwnedToolHooks,
  textToolResult,
...
describe("NexisClaw-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetNexisClawOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installNexisClawOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installNexisClawOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installNexisClawOwnedToolHooks();
