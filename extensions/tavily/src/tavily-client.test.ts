...
vi.mock("NexisClaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: NexisClaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "NexisClaw" });
  });

  it("runTavilyExtract sends X-Client-Source: NexisClaw", async () => {
