...
import type { MemorySearchResult } from "nexusclaw/plugin-sdk/memory-core-host-runtime-files";
import { formatMemoryDreamingDay } from "nexusclaw/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "nexusclaw/plugin-sdk/memory-host-events";
import { privateFileStore } from "nexusclaw/plugin-sdk/security-runtime";
...
const PROMOTION_MARKER_PREFIX = "nexusclaw-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*nexusclaw-memory-promotion:/i.test(snippet) ||
...
const DREAMING_FENCE_START_RE = /<!--\s*nexusclaw:dreaming:[a-z][a-z0-9-]*:start\s*-->/i;
const DREAMING_FENCE_END_RE = /<!--\s*nexusclaw:dreaming:[a-z][a-z0-9-]*:end\s*-->/i;
...
async function rehydratePromotionCandidate(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    // content. If rehydration lands inside an nexusclaw:dreaming fence (for example
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*nexusclaw-memory-promotion:([^\n]+?)\s*-->/gi);
