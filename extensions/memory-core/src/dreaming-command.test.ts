...
import type { NexusClawPluginApi } from "nexusclaw/plugin-sdk/plugin-entry";
...
function resolveStoredDreaming(config: NexusClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: NexusClawConfig = {}) {
  const registered: { command?: NexusClawPluginCommandDefinition } = {};
  let runtimeConfig: NexusClawConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: NexusClawConfig) => void }) => {
      ...
      replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: NexusClawConfig }) => {
        runtimeConfig = nextConfig;
      }),
      writeConfigFile: vi.fn(async (nextConfig: NexusClawConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  } as unknown as NexusClawPluginApi["runtime"];

  const api = {
    runtime,
    registerCommand: vi.fn((definition: NexusClawPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as NexusClawPluginApi;
