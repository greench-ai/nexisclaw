...
import { peekSystemEventEntries } from "nexusclaw/plugin-sdk/system-event-runtime";
...
export function registerShortTermPromotionDreaming(api: NexusClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): NexusClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as NexusClawConfig;
