...
} from "nexusclaw/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "nexusclaw/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "nexusclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "nexusclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "nexusclaw/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("nexusclaw.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("nexusclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-backfill-");
    ...
    expect(content.match(/nexusclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- nexusclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- nexusclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- nexusclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- nexusclaw:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- nexusclaw:dreaming:light:start -->\n- Candidate: test\n<!-- nexusclaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
        "# Dream Diary",
        "",
        "<!-- nexusclaw:dreaming:diary:start -->",
        "---",
        "",
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("nexusclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("nexusclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("nexusclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-detach-");
    const logger = createMockLogger();
