...
import * as memoryCoreHostRuntimeCoreModule from "NexisClaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "NexisClaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "NexisClaw/plugin-sdk/session-store-runtime";
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("NexisClaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("NexisClaw-dreaming-backfill-");
    ...
    expect(content.match(/NexisClaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("NexisClaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- NexisClaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- NexisClaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- NexisClaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- NexisClaw:dreaming:diary:end -->");
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("NexisClaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("NexisClaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("NexisClaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("NexisClaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("NexisClaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("NexisClaw-dreaming-detach-");
    const logger = createMockLogger();
