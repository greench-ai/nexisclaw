...
import { resolveMemoryDreamingConfig } from "nexusclaw/plugin-sdk/memory-core-host-status";
import type { NexusClawPluginApi, PluginCommandContext } from "nexusclaw/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "nexusclaw/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: NexusClawConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: NexusClawConfig, enabled: boolean): NexusClawConfig {
...
function formatStatus(cfg: NexusClawConfig): string {
...
export async function handleDreamingCommand(api: NexusClawPluginApi, ctx: PluginCommandContext) {
  const args = ctx.args?.trim() ?? "";
  const [firstToken = ""] = args
    .split(/\s+/)
    .filter(Boolean)
    .map((token) => normalizeLowercaseStringOrEmpty(token));
  const currentConfig = api.runtime.config.current() as NexusClawConfig;
...
export function registerDreamingCommand(api: NexusClawPluginApi): void {
  api.registerCommand({
    name: "dreaming",
