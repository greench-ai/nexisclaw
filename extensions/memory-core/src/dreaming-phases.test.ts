...
import type { NexusClawConfig } from "nexusclaw/plugin-sdk/config-contracts";
import { RequestScopedSubagentRuntimeError } from "nexusclaw/plugin-sdk/error-runtime";
import { resolveSessionTranscriptsDirForAgent } from "nexusclaw/plugin-sdk/memory-core-host-runtime-core";
import {
  resolveMemoryCorePluginConfig,
  resolveMemoryLightDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "nexusclaw/plugin-sdk/memory-core-host-status";
...
const LIGHT_DREAMING_TEST_CONFIG: NexusClawConfig = {
  plugins: {
    entries: {
      "memory-core": {
        config: {
...
async function createDreamingWorkspace(): Promise<string> {
  const workspaceDir = await createTempWorkspace("nexusclaw-dreaming-phases-");
...
async function triggerLightDreaming(
  ...
  await beforeAgentReply(
    { cleanedBody: "__nexusclaw_memory_core_light_sleep__" },
...
describe("memory-core dreaming phases", () => {
  it("uses the hashed narrative session key for sweep-level fallback cleanup", async () => {
    ...
    const testConfig: NexusClawConfig = {
      ...
      await beforeAgentReply(
        {
          cleanedBody: [
            "System: rotate logs",
            "System: __nexusclaw_memory_core_light_sleep__",
            "",
            "A scheduled reminder has been triggered. The reminder content is:",
            "",
            "rotate logs",
            "__nexusclaw_memory_core_light_sleep__",
            "",
  ...
  it("checkpoints session transcript ingestion and skips unchanged transcripts", async () => {
    ...
    vi.stubEnv("NEXUSCLAW_STATE_DIR", path.join(workspaceDir, ".state"));
  ...
  it("skips dreaming-generated narrative transcripts during session ingestion", async () => {
    ...
    await fs.writeFile(
      ...
      [
        JSON.stringify({
          type: "custom",
          customType: "nexusclaw:bootstrap-context:full",
          data: {
  ...
  it("records light/rem signals that reinforce deep promotion ranking", async () => {
    ...
    await withDreamingTestClock(async () => {
      ...
      await beforeAgentReply(
        { cleanedBody: "__nexusclaw_memory_core_rem_sleep__" },
        { trigger: "heartbeat", workspaceDir },
      );
    });
  ...
  it("increments dailyCount when the same daily file is re-ingested on a later day", async () => {
    ...
    const configForTest: NexusClawConfig = {
      plugins: {
        entries: {
          "memory-core": {
            config: {
