...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-NexisClaw-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-NexisClaw-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-NexisClaw-turn-attempt"]).toBe("2");
    expect(state?.metadata?.NexisClaw_session_id).toBe("session-123");
    expect(state?.metadata?.NexisClaw_turn_id).toBe("turn-123");
    expect(state?.metadata?.NexisClaw_turn_attempt).toBe("2");
    expect(state?.metadata?.NexisClaw_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-NexisClaw-session-id"]).toBe("session-123");
