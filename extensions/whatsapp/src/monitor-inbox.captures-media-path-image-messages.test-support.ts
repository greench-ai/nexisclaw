...
vi.mock("NexisClaw/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("NexisClaw/plugin-sdk/logging-core")>(
    "NexisClaw/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("NexisClaw WhatsApp listener close");
