...
import { resolveRequiredHomeDir, resolveStateDir } from "NexisClaw/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "NexisClaw-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("NexisClaw/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ?? (await import("NexisClaw/plugin-sdk/runtime")).createBackupArchive;
