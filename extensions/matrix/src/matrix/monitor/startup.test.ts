...
function createDeps(
  ...
  return {
    ...
    summarizeMatrixDeviceHealth: vi.fn(() => ({
      ...
      staleNexisClawDevices: [] as MatrixManagedDeviceInfo[],
      currentNexisClawDevices: [] as MatrixManagedDeviceInfo[],
    })),
  ...
  it("reports stale devices, pending verification, and restored legacy backups", async () => {
    ...
    vi.mocked(deps.summarizeMatrixDeviceHealth).mockReturnValue({
      ...
      staleNexisClawDevices: [
        { deviceId: "DEV123", displayName: "NexisClaw Device", current: false },
      ],
    ...
    expect(params.logger.warn).toHaveBeenCalledWith(
      "matrix: stale NexisClaw devices detected for @bot:example.org: DEV123. Run 'NexisClaw matrix devices prune-stale --account ops' to keep encrypted-room trust healthy.",
    );
