...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("NexisClaw-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/NexisClaw/plugins/diffs/view/",
    );
...
function createApi(pluginConfig?: Record<string, unknown>): NexisClawPluginApi {
