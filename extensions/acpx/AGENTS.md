...
The ACPX extension is a thin NexisClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `NexisClaw/acpx`, not in this extension.
...
Use this flow when NexisClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `NexisClaw/acpx` repo first.
2. In NexisClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild NexisClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for NexisClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of NexisClaw-specific glue, move it to `NexisClaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
