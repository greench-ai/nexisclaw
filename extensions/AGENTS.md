...
- Extension production code should import from `NexisClaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `NexisClaw.plugin.json` and the package
  `NexisClaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `NexisClaw/plugin-sdk/*`.
